...
void* function_call_param_iterator_c::visit(il_param_assignment_c* symbol) {
    ...
    if (!((NULL != symbol->simple_instr_list) ^ (NULL != symbol->il_operand)))
        ERROR;
    if (NULL != symbol->simple_instr_list)
        expression = symbol->simple_instr_list;
    if (NULL != symbol->il_operand)
        expression = symbol->il_operand;
...
void* function_call_param_iterator_c::visit(function_invocation_c* symbol) {
    ...
    if (symbol == (function_invocation_c*)f_call) {
        if (symbol->formal_param_list != NULL)
            return symbol->formal_param_list->accept(*this);
        if (symbol->nonformal_param_list != NULL)
            return symbol->nonformal_param_list->accept(*this);
    }
...
void* function_call_param_iterator_c::visit(output_variable_param_assignment_c* symbol) {
    ...
    if (NULL != symbol->not_param)
        ERROR;  // we do not yet support it, so it is best to simply stop than to fail silently...
