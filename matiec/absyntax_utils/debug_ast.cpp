...
static void dump_cvalue(const_value_c const_value) {
    ...
    if (const_value._real64.is_valid())
        fprintf(stderr, "%f", const_value._real64.get());
    else if (const_value._real64.is_overflow())
        fprintf(stderr, "ov");
    else if (const_value._real64.is_nonconst())
        fprintf(stderr, "nc");
    else
        fprintf(stderr, "?");
    fprintf(stderr, ", i=");
    if (const_value._int64v.is_valid())
        fprintf(stderr, "%" PRId64 "", const_value._int64v.get());
    else if (const_value._int64v.is_overflow())
        fprintf(stderr, "ov");
    else if (const_value._int64v.is_nonconst())
        fprintf(stderr, "nc");
    else
        fprintf(stderr, "?");
    fprintf(stderr, ", u=");
    if (const_value._uint64.is_valid())
        fprintf(stderr, "%" PRIu64 "", const_value._uint64.get());
    else if (const_value._uint64.is_overflow())
        fprintf(stderr, "ov");
    else if (const_value._uint64.is_nonconst())
        fprintf(stderr, "nc");
    else
        fprintf(stderr, "?");
    fprintf(stderr, ", b=");
    if (const_value._bool.is_valid())
        fprintf(stderr, "%d", const_value._bool.get() ? 1 : 0);
    else if (const_value._bool.is_overflow())
        fprintf(stderr, "ov");
    else if (const_value._bool.is_nonconst())
        fprintf(stderr, "nc");
    else
        fprintf(stderr, "?");
    fprintf(stderr, "}");
...
void* print_symbol_c::visit(il_instruction_c* symbol) {
    ...
    /* NOTE: std::map.size() returns a size_type, whose type is dependent on compiler/platform. To be portable, we need
     * to do an explicit type cast. */
...
void print_ast_c::prefix_fcall(symbol_c* symbol) {
