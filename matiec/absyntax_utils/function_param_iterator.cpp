...
#include "function_param_iterator.h" /* no longer required, aready included by absyntax_utils.hh */
...
/* NOTE: The following function is not really needed, as we could get the value that constant_folding_c determined for
 * this integer. Remember that currently constant_folding_c runs before this class is ever used/called! However, I
 * (Mario) do not currently feel it would be a good idea to restrict the use of this abstract syntax utility to only
 * after the constant_folding_c has had a chance to fill in the constant value of this symbol. For this reason only, I
 * have opted to let this abstract syntax utility have its own private copy of the extract_integer() function. Another
 * aspect that makes this OK is that this function will only be used to extract the integer value of the index for the
 * first extensible paramater (examples follow shortly). Since this is an extension to IEC 61131-3 that we created to
 * allow us to handle extensible functions with very little hard coding, it is OK if we impose extra/different limits on
 * how an integer may be legally be formated in this case. This will also only show up in code that describes the
 * interface to the standard function of IEC 61131-3, which the user will not ever get to see. We write that IEC 61131-3
 * code ourselves!
 *
...
static int extract_first_index_value(symbol_c* sym) {
    ...
    if (errno != 0)
        ERROR;
    if (ret < 0)
        ERROR;  // the following code assumes that the first index will never be negative!
    if (ret > std::numeric_limits<int>::max())
        ERROR;  // output of this function is only an int!!
...
void* function_param_iterator_c::visit(input_declaration_list_c* symbol) {
    TRACE("input_declaration_list_c");
    return iterate_list(symbol);
}

void* function_param_iterator_c::visit(edge_declaration_c* symbol) {
...
void* function_param_iterator_c::visit(en_param_declaration_c* symbol) {
    ...
    /* If we have found the parameter we will be returning, we set the en_eno_param_implicit to TRUE if implicitly
     * defined */
    if (res != NULL)
...
void* function_param_iterator_c::visit(fb_name_list_c* symbol) {
...
void* function_param_iterator_c::visit(var_declaration_list_c* symbol) {
...
void* function_param_iterator_c::visit(var_declarations_c* symbol) {
...
void* function_param_iterator_c::visit(external_declaration_list_c* symbol) {
...
void* function_param_iterator_c::visit(var_init_decl_list_c* symbol) {
...
/* B 1.5.1 - Functions */
/***********************/
void* function_param_iterator_c::visit(function_declaration_c* symbol) {
    TRACE("function_declaration_c");
    return symbol->var_declarations_list->accept(*this);
}
/* intermediate helper symbol for function_declaration */
void* function_param_iterator_c::visit(var_declarations_list_c* symbol) {
    TRACE("var_declarations_list_c");
    return iterate_list(symbol);
}
void* function_param_iterator_c::visit(function_var_decls_c* symbol) {
    TRACE("function_var_decls_c"); /* ignore */
    return NULL;
}
...
void* function_param_iterator_c::visit(function_block_declaration_c* symbol) {
...
void* function_param_iterator_c::visit(temp_var_decls_c* symbol) {
    TRACE("temp_var_decls_c"); /* ignore */
    return NULL;
}
void* function_param_iterator_c::visit(temp_var_decls_list_c* symbol) {
    TRACE("temp_var_decls_list_c"); /* ignore */
    return NULL;
}

/*  VAR NON_RETAIN var_init_decl_list END_VAR */
void* function_param_iterator_c::visit(non_retentive_var_decls_c* symbol) {
    TRACE("non_retentive_var_decls_c"); /* ignore */
    return NULL;
}
...
void* function_param_iterator_c::visit(program_declaration_c* symbol) {
