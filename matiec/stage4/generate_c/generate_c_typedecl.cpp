...
public:
    ...
    // by default generate an ERROR if a visit method is called, unless it is explicitly handled in
    // generate_datatypes_aliasid_c
    ...
    static identifier_c* create_id(symbol_c* symbol) {
        if (NULL == singleton_)
            singleton_ = new generate_datatypes_aliasid_c();
        if (NULL == singleton_)
            ERROR;
    ...
    void* visit(array_specification_c* symbol) {
        ...
        if (get_datatype_info_c::is_ref_to(symbol->non_generic_type_name) &&
             ...
             */
            current_array_name += "__REF_TO_";
            current_array_name += get_datatype_info_c::get_id_str(
                get_datatype_info_c::get_ref_to(symbol->non_generic_type_name));
        } else {
...
protected:
    /* The following member variable is completely useless - the s4o variable inherited from
     * generate_c_base_and_typeid_c could be used to the same effect. We keep it here merely because this
     * generate_c_typedecl_c will typically be called with s4o referencing an include file (typically POUS.h), and using
     * s4o_incl throughout this code will help the reader of the code to keep this fact in mind.
     */
...
private:
    ...
    /* Although this generate_c_typedecl_c inherits directly from generate_c_base_and_typeid_c, we still need an
     * independent instance of that base class. This is because generate_c_typedecl_c will overload some of the visitors
     * in the base class generate_c_base_and_typeid_c. When we want the to use the version of these visitors() in
     * generate_c_typedecl_c,        we call accept(*this); When we want the to use the version of these visitors() in
     * generate_c_base_and_typeid_c, we call accept(*generate_c_typeid);
     */
    generate_c_base_and_typeid_c* generate_c_typeid;

public:
    generate_c_typedecl_c(stage4out_c* s4o_ptr)
        : generate_c_base_and_typeid_c(s4o_ptr), s4o_incl(*s4o_ptr) /*, generate_c_print_typename(s4o_ptr) */ {
        ...
        if (current_typedefinition == subrange_td) {
            ...
            case subrangetest_bd:
                if (symbol->subrange != NULL) {
                    ...
                    /* NOTE: IEC 61131-3 v2 syntax mandates that the integer type name be one of SINT, ..., LINT, USINT,
                     * ... ULIT */
                    /*       For this reason, the following condition will always be false, and therefore this is a
                     * block of dead code. However, let's not delete it for now. It might come in useful for IEC 61131-3
                     * v3. For the moment, we just comment it out!
                     */
                    /*
    ...
    void* visit(enumerated_spec_init_c* symbol) {
        ...
        if (current_typedefinition == enumerated_td)
            symbol->enumerated_specification->accept(
                *this);  // always calls enumerated_value_list_c or derived_datatype_identifier_c
        else
    ...
    */
    ...
    void* visit(array_type_declaration_c* symbol) {
        ...
        // NOTE: remeber that symbol->array_spec_init may point to a derived_datatype_identifier_c, which is why we use
        // symbol->array_spec_init->datatype instead!
        if (NULL == symbol->array_spec_init->datatype)
            ERROR;
        ...
        symbol->array_spec_init->anotations_map["generate_c_annotaton__implicit_type_id"] =
            id;  // probably not needed, bu let's play safe.
    ...
    void* visit(array_spec_init_c* symbol) {
        ...
        symbol->array_specification->accept(
            *this);  // always calls array_specification_c or derived_datatype_identifier_c
        return NULL;
    }
    ...
    void* visit(array_specification_c* symbol) {
        ...
        symbol->array_subrange_list->accept(
            *this);  // always calls array_subrange_list_c, which the iterator_visitor_c base class will call subrange_c
        current_basetypedeclaration = none_bd;
        return NULL;
    }
    ...
    void* visit(type_declaration_list_c* symbol) {
        ...
        return print_list_incl(symbol,
                               "",
                               "\n",
                               "\n");  // will always call string_type_declaration_c, structure_type_declaration_c,
                                       // array_type_declaration_c, simple_type_declaration_c,
                                       // subrange_type_declaration_c, enumerated_type_declaration_c, ref_type_decl_c
    }
...
SYM_REF2(array_initial_elements_c, integer, array_initial_element)
    ...
    void* visit(structure_type_declaration_c* symbol) {
        ...
        s4o_incl.print(",");
        symbol->structure_specification->accept(
            *this);  // always calls initialized_structure_c or structure_element_declaration_list
        s4o_incl.print(")\n");
 ...
 *
 * e.g.:
 *      VAR  x: ARRAY [1..3] OF INT; END_VAR   <---- ARRAY  datatype is implicitly declared inside the variable
 * declaration VAR  y: REF_TO INT;          END_VAR   <---- REF_TO datatype is implicitly declared inside the variable
 * declaration TYPE STRUCT a: ARRAY [1..3] OF INT;       <---- ARRAY  datatype is implicitly declared inside the struct
 * type declaration b: REF_TO INT;                <---- REF_TO datatype is implicitly declared inside the struct type
 * declaration c: INT; END_STRUCT END_TYPE
 */
...
public:
    ...
    void* visit(array_type_declaration_c* symbol) {
        return NULL;
    }  // This is not an implicitly defined array!
    ...
    void* visit(ref_spec_init_c* symbol) {
        ...
        if (implicit_id_count > 1)
            ERROR;
        if (implicit_id_count == 1)
            symbol->anotations_map["generate_c_annotaton__implicit_type_id"] =
    ...
    void* visit(ref_type_decl_c* symbol) {
        return NULL;
    }  // This is not an implicitly defined REF_TO!
    ...
    void* visit(array_spec_init_c* symbol) {
        symbol->array_specification->accept(
            *this);  //--> always calls array_specification_c or derived_datatype_identifier_c
        int implicit_id_count = symbol->array_specification->anotations_map.count(
            "generate_c_annotaton__implicit_type_id");
        if (implicit_id_count > 1)
            ERROR;
        if (implicit_id_count == 1)
