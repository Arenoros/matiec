...
/* Macros to access the constant value of each expression (if it exists) from the annotation introduced to the symbol_c
 * object by constant_folding_c in stage3! */
...
int stage4_parse_options(char* options) {
    enum
    {
        ...
        char* const token[] = {/*       LINE_OPT*/ (char*)"l",
                               /*   SEPTFILE_OPT*/ (char*)"p",
                               /*     BACKUP_OPT*/ (char*)"b",
                               /* SOME_OTHER_OPT, ...             */
                               NULL};
        ...
        while (*subopts != '\0') {
            ...
            case LINE_OPT:
                generate_line_directives__ = 1;
                break;
            case SEPTFILE_OPT:
                generate_pou_filepairs__ = 1;
                break;
            case BACKUP_OPT:
                generate_plc_state_backup_fuctions__ = 1;
                break;
            default:
                fprintf(stderr, "Unrecognized option: -O %s\n", value);
                return -1;
                break;
            }
        }
...
#include "generate_c_base.cpp"
#include "generate_c_typedecl.cpp"
#include "generate_c_sfcdecl.cpp"
#include "generate_c_vardecl.cpp"
#include "generate_c_configbody.cpp"
#include "generate_location_list.cpp"
#include "generate_var_list.cpp"
...
public:
    print_function_parameter_data_types_c(stage4out_c* s4o_ptr): generate_c_base_and_typeid_c(s4o_ptr) {
        current_type = NULL;
    }
     ...
     * END_FUNCTION */
    /* | FUNCTION derived_function_name ':' derived_type_name io_OR_function_var_declarations_list function_body
     * END_FUNCTION */
...
public:
    ...
    static bool is_complex_type(symbol_c* symbol) {
        ...
        if (!get_datatype_info_c::is_type_valid(symbol->datatype))
            return false;
        return (get_datatype_info_c::is_structure(symbol->datatype) || get_datatype_info_c::is_array(symbol->datatype));
    }
...
public:
    /* returns the first element (from left to right) in a structured variable that is not a FB, i.e. is either a
    ...
    static symbol_c* find_first_nonfb(symbol_c* symbol) {
        if (NULL == singleton_)
            singleton_ = new analyse_variable_c();
        if (NULL == singleton_)
            ERROR;
        if (NULL == symbol)
            ERROR;
...
#include "generate_c_st.cpp"
#include "generate_c_il.cpp"
#include "generate_c_inlinefcall.cpp"
...
unsigned long long calculate_time(symbol_c* symbol) {
    ...
    if ((NULL == interval) && (NULL == duration)) {
        STAGE4_ERROR(symbol, symbol, "This type of interval value is not currently supported");
        ERROR;
    }
    ...
    if (NULL != duration) {
        ...
        if (duration->neg != NULL) {
            STAGE4_ERROR(duration, duration, "Negative TIME literals for interval are not currently supported");
            ERROR;
        }
    ...
    if (NULL != interval) {
        ...
        */
        ...
        if (NULL != interval->milliseconds) {
            if (VALID_CVALUE(int64v, interval->milliseconds) && GET_CVALUE(int64v, interval->milliseconds) < 0)
                ERROR;  // interval elements should always be positive!
            if (VALID_CVALUE(int64v, interval->milliseconds))
                time_ull += GET_CVALUE(int64v, interval->milliseconds) * MILLISECOND;
            else if (VALID_CVALUE(uint64, interval->milliseconds))
                time_ull += GET_CVALUE(uint64, interval->milliseconds) * MILLISECOND;
            else if (VALID_CVALUE(real64, interval->milliseconds))
                time_ld += GET_CVALUE(real64, interval->milliseconds) * MILLISECOND;
            else
                ERROR;  // if (NULL != interval->milliseconds) is true, then it must have a valid constant value!
        }

        if (NULL != interval->seconds) {
            if (VALID_CVALUE(int64v, interval->seconds) && GET_CVALUE(int64v, interval->seconds) < 0)
                ERROR;  // interval elements should always be positive!
            if (VALID_CVALUE(int64v, interval->seconds))
                time_ull += GET_CVALUE(int64v, interval->seconds) * SECOND;
            else if (VALID_CVALUE(uint64, interval->seconds))
                time_ull += GET_CVALUE(uint64, interval->seconds) * SECOND;
            else if (VALID_CVALUE(real64, interval->seconds))
                time_ld += GET_CVALUE(real64, interval->seconds) * SECOND;
            else
                ERROR;  // if (NULL != interval->seconds) is true, then it must have a valid constant value!
        }

        if (NULL != interval->minutes) {
            if (VALID_CVALUE(int64v, interval->minutes) && GET_CVALUE(int64v, interval->minutes) < 0)
                ERROR;  // interval elements should always be positive!
            if (VALID_CVALUE(int64v, interval->minutes))
                time_ull += GET_CVALUE(int64v, interval->minutes) * SECOND * 60;
            else if (VALID_CVALUE(uint64, interval->minutes))
                time_ull += GET_CVALUE(uint64, interval->minutes) * SECOND * 60;
            else if (VALID_CVALUE(real64, interval->minutes))
                time_ld += GET_CVALUE(real64, interval->minutes) * SECOND * 60;
            else
                ERROR;  // if (NULL != interval->minutes) is true, then it must have a valid constant value!
        }

        if (NULL != interval->hours) {
            if (VALID_CVALUE(int64v, interval->hours) && GET_CVALUE(int64v, interval->hours) < 0)
                ERROR;  // interval elements should always be positive!
            if (VALID_CVALUE(int64v, interval->hours))
                time_ull += GET_CVALUE(int64v, interval->hours) * SECOND * 60 * 60;
            else if (VALID_CVALUE(uint64, interval->hours))
                time_ull += GET_CVALUE(uint64, interval->hours) * SECOND * 60 * 60;
            else if (VALID_CVALUE(real64, interval->hours))
                time_ld += GET_CVALUE(real64, interval->hours) * SECOND * 60 * 60;
            else
                ERROR;  // if (NULL != interval->hours) is true, then it must have a valid constant value!
        }

        if (NULL != interval->days) {
            if (VALID_CVALUE(int64v, interval->days) && GET_CVALUE(int64v, interval->days) < 0)
                ERROR;  // interval elements should always be positive!
            if (VALID_CVALUE(int64v, interval->days))
                time_ull += GET_CVALUE(int64v, interval->days) * SECOND * 60 * 60 * 24;
            else if (VALID_CVALUE(uint64, interval->days))
                time_ull += GET_CVALUE(uint64, interval->days) * SECOND * 60 * 60 * 24;
            else if (VALID_CVALUE(real64, interval->days))
                time_ld += GET_CVALUE(real64, interval->days) * SECOND * 60 * 60 * 24;
            else
                ERROR;  // if (NULL != interval->days) is true, then it must have a valid constant value!
        }
...
#include "generate_c_sfc.cpp"

generate_c_SFC_IL_ST_c::generate_c_SFC_IL_ST_c(stage4out_c* s4o_ptr,
                                               symbol_c* name,
                                               symbol_c* scope,
                                               const char* variable_prefix) {
    if (NULL == scope)
    ...
    static void handle_function(function_declaration_c* symbol, stage4out_c& s4o, bool print_declaration) {
        ...
        vardecl = new generate_c_vardecl_c(&s4o,
                                           ...
                                           generate_c_vardecl_c::input_vt | generate_c_vardecl_c::output_vt |
                                               generate_c_vardecl_c::inoutput_vt | generate_c_vardecl_c::en_vt |
        ...
        vardecl = new generate_c_vardecl_c(&s4o,
                                           ...
                                           generate_c_vardecl_c::output_vt | generate_c_vardecl_c::inoutput_vt |
                                               generate_c_vardecl_c::private_vt | generate_c_vardecl_c::eno_vt);
        vardecl->print(symbol->var_declarations_list);
        delete vardecl;
         ...
         */
        if (!get_datatype_info_c::is_VOID(
                symbol->type_name->datatype)) {  // only print return variable if return datatype is not VOID
            s4o.print(s4o.indent_spaces);
            symbol->type_name->accept(print_base); /* return type */
            s4o.print(" ");
            symbol->derived_function_name->accept(print_base);
            s4o.print(" = ");
            {
                ...
                case initialization_analyzer_c::struct_it: {
                    generate_c_structure_initialization_c* structure_initialization =
                        new generate_c_structure_initialization_c(&s4o);
                ...
                case initialization_analyzer_c::array_it: {
                    generate_c_array_initialization_c* array_initialization = new generate_c_array_initialization_c(
                        &s4o);
        ...
        if (!get_datatype_info_c::is_VOID(
                symbol->type_name->datatype)) {  // only print 'return <fname>' if return datatype is not VOID
            s4o.print(s4o.indent_spaces + "return ");
            symbol->derived_function_name->accept(print_base);
            s4o.print(";\n");
        }
...
public:
    ...
    static void handle_function_block(function_block_declaration_c* symbol, stage4out_c& s4o, bool print_declaration) {
        ...
        if (print_declaration) {
            ...
            vardecl = new generate_c_vardecl_c(&s4o,
                                               generate_c_vardecl_c::local_vf,
                                               generate_c_vardecl_c::temp_vt | generate_c_vardecl_c::private_vt |
        ...
        if (!print_declaration) {
            ...
            generate_c_inlinefcall_c* inlinedecl = new generate_c_inlinefcall_c(&s4o,
                                                                                symbol->fblock_name,
                                                                                symbol,
                                                                                FB_FUNCTION_PARAM "->");
        ...
        if (print_declaration) {
            ...
            vardecl = new generate_c_vardecl_c(&s4o,
                                               ...
                                               generate_c_vardecl_c::input_vt | generate_c_vardecl_c::output_vt |
                                                   ...
                                                   generate_c_vardecl_c::private_vt | generate_c_vardecl_c::located_vt |
                                                   generate_c_vardecl_c::external_vt | generate_c_vardecl_c::en_vt |
                                                   generate_c_vardecl_c::eno_vt);
        ...
        if (print_declaration) {
            ...
            vardecl = new generate_c_vardecl_c(&s4o, generate_c_vardecl_c::init_vf, generate_c_vardecl_c::temp_vt);
            vardecl->print(symbol->var_declarations, NULL, FB_FUNCTION_PARAM "->");
            delete vardecl;
            s4o.print("\n");
...
public:
    ...
    */
    /*
    SYM_REF6(configuration_declaration_c, configuration_name, global_var_declarations, resource_declarations,
    access_declarations, instance_specific_initializations, unused)
    */
...
public:
    generate_c_resources_c(stage4out_c* s4o_ptr,
                           symbol_c* config_scope,
                           symbol_c* resource_scope,
    ...
    */
     ...
     */
    // SYM_REF6(program_configuration_c, retain_option, program_name, task_name, program_type_name, prog_conf_elements,
    // unused)
    ...
    void* visit(task_initialization_c* symbol) {
        ...
        case run_dt:
            if (symbol->single_data_source != NULL) {
                ...
                s4o.print(s4o.indent_spaces + "{");
                symbol_c* current_var_reference = ((global_var_reference_c*)(symbol->single_data_source))
                                                      ->global_var_name;
...
public:
    generate_c_backup_resource_decl_c(stage4out_c* s4o_ptr): generate_c_base_and_typeid_c(s4o_ptr){};
...
public:
    generate_c_backup_config_c(stage4out_c* s4o_ptr): generate_c_base_and_typeid_c(s4o_ptr){};
...
public:
    generate_c_backup_resource_c(stage4out_c* s4o_ptr): generate_c_base_and_typeid_c(s4o_ptr){};
...
public:
    generate_c_c(stage4out_c* s4o_ptr, const char* builddir)
        : s4o(*s4o_ptr), pous_s4o(builddir, "POUS", "c"), pous_incl_s4o(builddir, "POUS", "h"),
          located_variables_s4o(builddir, "LOCATED_VARIABLES", "h"), variables_s4o(builddir, "VARIABLES", "csv"),
          generate_c_typedecl(&pous_incl_s4o), generate_c_implicit_typedecl(&pous_incl_s4o, &generate_c_typedecl) {
    ...
    /* In the derived datatype and POUs declarations, the names are stored as identfier_c, so the following visitors are
     * not required! */
 ...
 *          specially created stage4out_c (s4o_c and s4o_h) will not comply with the
 * enable/disable_code_generation_pragma_c
 */
#define handle_pou(fname, pname)                                                                                       \
    ...
    if (generate_pou_filepairs__) {                                                                                    \
        ...
        s4o_c.print("#include \"");                                                                                    \
        s4o_c.print(pou_name);                                                                                         \
        s4o_c.print(".h\"\n");                                                                                         \
        s4o_h.print("#ifndef __");                                                                                     \
        s4o_h.print(pou_name);                                                                                         \
        s4o_h.print("_H\n");                                                                                           \
        s4o_h.print("#define __");                                                                                     \
        s4o_h.print(pou_name);                                                                                         \
        s4o_h.print("_H\n");                                                                                           \
        generate_c_implicit_typedecl_c generate_c_implicit_typedecl__(&s4o_h);                                         \
        symbol->accept(                                                                                                \
            generate_c_implicit_typedecl__); /* generate implicitly delcared datatypes (arrays and ref_to) */          \
    ...
    /***********************/
    void* visit(function_declaration_c* symbol) {
        handle_pou(handle_function, symbol->derived_function_name) return NULL;
    }
    ...
    void* visit(function_block_declaration_c* symbol) {
        handle_pou(handle_function_block, symbol->fblock_name) return NULL;
    }
    ...
    void* visit(program_declaration_c* symbol) {
        handle_pou(handle_program, symbol->program_type_name) return NULL;
    }
    ...
    void* visit(configuration_declaration_c* symbol) {
        ...
        if (configuration_count++) {
            ...
            STAGE4_ERROR(symbol,
                         symbol,
                         "A previous CONFIGURATION has already been declared (C code generation currently only allows "
...
visitor_c* new_code_generator(stage4out_c* s4o, const char* builddir) {
    return new generate_c_c(s4o, builddir);
}
