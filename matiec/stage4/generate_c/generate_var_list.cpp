...
public:
    ...
    locationtype_t get_location_type(symbol_c* symbol) {
        ...
        symbol->accept(*this);
        if (current_location_type == none_lt)
            ERROR;
...
public:
    generate_var_list_c(stage4out_c* s4o_ptr, symbol_c* scope): generate_c_base_and_typeid_c(s4o_ptr) {
    ...
    void* visit(enumerated_spec_init_c* symbol) {
        /* search_base_type_c now returns an enumerated_type_declaration_c as the base type of a non-anonymous
         * enumerated type (non-anonymous means it is declared inside a TYPE ... END_TYPE declaration, with a given
         * name/identifier unlike implicitly defined anonymous datatypes declared inside VAR ... END_VAR declarations!).
