...
private:
     ...
     * The variable_prefix_ variable will contain the correct string which needs to be prefixed to all variable
     * accesses. This string is set with the set_variable_prefix() member function.
     */
...
public:
    ...
    void set_variable_prefix(const char* variable_prefix) {
        variable_prefix_ = variable_prefix;
    }
    const char* get_variable_prefix(void) {
        return variable_prefix_;
    }
    bool is_variable_prefix_null(void) {
        return variable_prefix_ == NULL;
    }
    ...
    void print_line_directive(symbol_c* symbol) {
        if (!generate_line_directives__)
            return; /* global variable generate_line_directives__ is defined in generate_c.cc */
        s4o.print("#line ");
    ...
    void* print_striped_token(token_c* token, int offset = 0) {
        ...
        for (unsigned int i = offset; i < strlen(token->value); i++) {
            if (leading_zero && (token->value[i] != '0' || i == strlen(token->value) - 1 || token->value[i + 1] == '.'))
                leading_zero = false;
    ...
    void* print_list(list_c* list,
        ...
        if (list->size() > 0) {
            // std::cout << "generate_c_base_c::print_list(n = " << list->size() << ")   000\n";
    ...
    void* print_binary_expression(symbol_c* l_exp, symbol_c* r_exp, const char* operation) {
        s4o.print("(");
        l_exp->accept(*this);
        s4o.print(operation);
        r_exp->accept(*this);
    ...
    void* print_unary_expression(symbol_c* exp, const char* operation) {
        s4o.print(operation);
        s4o.print("(");
        exp->accept(*this);
        s4o.print(")");
    ...
    void* print_binary_function(const char* function, symbol_c* l_exp, symbol_c* r_exp) {
        s4o.print(function);
        s4o.print("(");
        l_exp->accept(*this);
        s4o.print(", ");
     ...
     *         3rd parameter: number of operands we will be passing (required because we are calling an extensible
     * standard function!) 4th parameter: the left  hand side of the comparison expression (in out case, the IL implicit
     * variable) 4th parameter: the right hand side of the comparison expression (in out case, current operand)
     *
     *         The 1st and 2nd parameter may not be present, only issue them if NE and ENO are being generated!
     *         The 3rd parameter must not be generated when the 'NE' function is called (it is not an extensible
     * function!)
     *
     ...
     */
    void* print_compare_function(const char* function, symbol_c* compare_type, symbol_c* l_exp, symbol_c* r_exp) {
    ...
    void* print_check_function(symbol_c* type, symbol_c* value, symbol_c* fb_name = NULL, bool temp = false) {
    ...
    void* visit(neg_time_c* symbol) {
        s4o.print("-1"); /* negative time value */
        ;
        return NULL;
    }
    ...
    void* visit(symbolic_constant_c* symbol) {
        TRACE("symbolic_variable_c");
        if (symbol->const_value._int64v.is_valid())
            s4o.print(symbol->const_value._int64v.get());
        else if (symbol->const_value._uint64.is_valid())
            s4o.print(symbol->const_value._uint64.get());
        else
            ERROR;
        return NULL;
    }
    ...
    // TODO:  It seems to me this code no longer gets to execute, since the function is overloaded in generate_c_st_c
    // and generate_c_il_c
...
public:
    ...
    */
    ...
    void* visit(ref_spec_c* symbol) {
         ...
         * *), through the visit(ref_spec_init_c*)), so we need to simply print out the name of the datatype we
         * reference to.
         */
