...
public:
    ...
    void* visit(array_specification_c* symbol) {
        ...
        case typedecl_am: {
            ...
            if (implicit_id_count > 1)
                ERROR;
            if (implicit_id_count == 1)
                /* this is part of an implicitly declared datatype (i.e. inside a variable decaration), for which an
                 * equivalent C datatype has already been defined. So, we simly print out the id of that C datatpe...
                 */
    ...
    void* visit(subrange_c* symbol) {
        ...
        case arraysize_am:
            ...
            if ((std::numeric_limits<unsigned long long int>::max() / array_size) < symbol->dimension)
                STAGE4_ERROR(symbol,
                             symbol,
                             "The array containing this subrange has a total number of elements larger than the "
                             "maximum currently supported (%llu).",
        ...
        case initializationvalue_am:
            ...
            for (int i = 0; i < symbol->size(); i++) {
                if (current_initialization_count >= defined_values_count) {
                    ...
                    array_initial_elements_c* array_initial_element = dynamic_cast<array_initial_elements_c*>(
                        symbol->get_element(i));
    ...
    void* visit(array_initial_elements_c* symbol) {
        ...
        if (std::numeric_limits<uint64_t>::max() > std::numeric_limits<unsigned long long int>::max())
            ERROR_MSG(
                "Assertion (sizeof(uint64_t) > sizeof(unsigned long long int)) failed! Compiler cannot execute "
                "correctly on the current platform!");

        switch (current_mode) {
        case initializationvalue_am:
            if (VALID_CVALUE(int64v, symbol->integer) && (GET_CVALUE(int64v, symbol->integer) >= 0))
                initial_element_count = GET_CVALUE(int64v, symbol->integer);
...
private:
    ...
    void update_type_init(symbol_c* symbol /* a spec_init_c, subrange_spec_init_c, etc... */) {
        ...
        if (NULL == this->current_var_init_symbol) {
            ...
            /* Note that Function Block 'data types' do not have a default value, so we cannot abort if no default value
             * is found! */
            /*
            if (NULL == this->current_var_init_symbol)
              ERROR;
        ...
        /* When handling arrays we must make sure that we use the base datatype, since arrays use an aliased name in the
         * C code! This is done using a stage4 annotation (on the base datatype class) named
         * "generate_c_annotaton__implicit_type_id" Note that we do this only _after_ determining the initial value,
         * since in principle the derived array could have a default initial different to the base array datatype!
         */
        if (NULL == this->current_var_type_symbol->datatype) {
            debug_c::print(this->current_var_type_symbol);
            ERROR;
        }
    ...
    void print_fb_explicit_initial_values(symbol_c* fbvar_name, symbol_c* init_values_list) {
        structure_element_initialization_list_c* init_list = dynamic_cast<structure_element_initialization_list_c*>(
            init_values_list);
        if (NULL == init_list)
            ERROR;

        for (int i = 0; i < init_list->size(); i++) {
            structure_element_initialization_c* init_list_elem = dynamic_cast<structure_element_initialization_c*>(
                init_list->get_element(i));
            if (NULL == init_list_elem)
                ERROR;
    ...
    void* declare_variables(symbol_c* symbol, bool is_fb = false) {
        ...
        if ((wanted_varformat == local_vf) || (wanted_varformat == init_vf) || (wanted_varformat == localinit_vf)) {
            for (int i = 0; i < list->size(); i++) {
                ...
                if (wanted_varformat != local_vf) {
                    if (wanted_varformat == localinit_vf && (current_vartype & inoutput_vt) != 0) {
                        s4o.print(";\n");
                        s4o.print(s4o.indent_spaces);
                        s4o.print("if (__");
                        list->get_element(i)->accept(*this);
...
protected:
    ...
    void* visit(array_var_init_decl_c* symbol) {
        ...
        if (wanted_varformat == constructorinit_vf) {
            ...
            array_initialization->init_array(symbol->var1_list,
                                             this->current_var_type_symbol,
                                             this->current_var_init_symbol);
            delete array_initialization;
        } else
            symbol->var1_list->accept(*this);
    ...
    void* visit(structured_var_init_decl_c* symbol) {
        ...
        if (wanted_varformat == constructorinit_vf) {
            ...
            structure_initialization->init_structure(symbol->var1_list,
                                                     this->current_var_type_symbol,
                                                     this->current_var_init_symbol);
            delete structure_initialization;
        } else
    ...
    void* visit(external_declaration_c* symbol) {
        ...
        if (!get_datatype_info_c::is_type_valid(this->current_var_type_symbol))
            ERROR;
