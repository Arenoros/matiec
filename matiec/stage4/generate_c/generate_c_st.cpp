...
private:
    ...
    void* print_setter(symbol_c* symbol,
        ...
        if (fb_symbol == NULL) {
            ...
            if (first_nonfb == NULL)
                ERROR;
            if (vartype == search_var_instance_decl_c::external_vt) {
                if (!get_datatype_info_c::is_type_valid(first_nonfb->datatype))
                    ERROR;
                if (get_datatype_info_c::is_function_block(
                        first_nonfb->datatype))  // handle situation where we are copying a complete fb -> fb1.fb2.fb3
                                                 // := fb4 (and fb3 is external!)
    ...
    void* visit(structured_variable_c* symbol) {
        ...
        case complextype_base_vg:
            ...
            /* NOTE: The following test includes a special case for SFC Steps. They are currently mapped onto a C data
             * structure that does not follow the standard IEC_<typename> pattern used for user defined structure
             * datatypes (i.e. it does not include the 'values' and 'flag' structure elements that are tested by
             * __GET_VAR and __SET_VAR acessor macros defined in acessor.h). However, the STEP.T and STEP.X elements of
             * this step structure are of the IEC_BOOL and IEC_TIME datatypes, and are actually structures that do have
             * the 'value' and 'flags' elements. So, it is safe to say that any  variable that is a STEPname is not of a
             * complex type, as its .T and .X elements are and can later be safely accessed using the __SET_VAR and
             * __GET_VAR macros.
             *
             *        For the above reason, a STEP must be handled as a FB, i.e. it does NOT contain the 'flags' and
             * 'value' elements!
             */
            if (get_datatype_info_c::is_function_block(symbol->record_variable->datatype) ||
                ...
                if (NULL == symbol->record_variable->scope)
                    ERROR;
                search_var_instance_decl_c search_var_instance_decl(symbol->record_variable->scope);
                if (search_var_instance_decl_c::external_vt ==
                    search_var_instance_decl.get_vartype(get_var_name_c::get_last_field(symbol->record_variable)))
                    s4o.print("->");
                else if (dynamic_cast<deref_operator_c*>(symbol->record_variable) != NULL)
                    s4o.print("->"); /* please read the comment in visit(deref_operator_c *) tio understand what this
                                        line is doing! */
                else
                    s4o.print(".");
        ...
        case complextype_suffix_vg:
            ...
            if (!(get_datatype_info_c::is_function_block(
                      symbol->record_variable->datatype)  // if the record variable is not a FB...
                  || get_datatype_info_c::is_sfc_step(
                         symbol->record_variable
                             ->datatype))) {  // ...nor an SFC step name, then it will certainly be a structure!
        ...
        default:
            if (this->is_variable_prefix_null()) {
                /* We are writing code for a FUNCTION. In this case, deref_operator_c are not transformed into the C
                 * pointer derefence syntax '->' (e.g. ptr->elem). We use instead the '*' syntax (e.g. (*ptr).elem)
                 * While in FB the '->' is generated by this structured_variable_c visitor, in Functions the '*' syntax
                 * is generated by the deref_operator_c visitor This is why here we do NOT have --> {if
                 * (dynamic_cast<deref_operator_c *>(symbol->record_variable) != NULL)  ..}
                 *
    ...
    void* visit(array_variable_c* symbol) {
        ...
        case complextype_suffix_vg:
            ...
            if (dynamic_cast<deref_operator_c*>(symbol->subscripted_variable) != NULL)
                s4o.print("->"); /* please read the comment in visit(deref_operator_c *) tio understand what this line
                                    is doing! */
            else
                s4o.print(".");
            s4o.print("table");
    ...
    void* visit(structure_element_initialization_list_c* symbol) {
        generate_c_structure_initialization_c* structure_initialization = new generate_c_structure_initialization_c(
            &s4o);
    ...
    void* visit(deref_operator_c* symbol) {
         ...
         * When producing C code for a FUNCTION_BLOCK, the deref_operator_c are transformed in two ways, depending on
         * where they occur.
         ...
         *         was the GET_VAR and SET_VAR are transformed into C, this does not work for '^' between a struct and
         * its element (e.g. struct_ref^.elem), which is whey in this case only we use the '->' syntax. NOTE: The use of
         * the -> syntax means that pointers to pointers are not supported betweem a struct and its elem (e.g. .
         * struct_ref_ref^^.elem) as this would be transformed into the invalid C code struct_ref_ref->->elem. This is
         * why we add a test for this case, and bug out with an error if we encounter it!!
         */
        if (this->is_variable_prefix_null()) {
            ...
            if ((NULL == dynamic_cast<structured_variable_c*>(symbol->parent)) &&
                ...
                /* We are in a structured variable - the structured_variable_c or the array_variable_c will already have
                 * printed out the '->' !! */
                if (NULL != dynamic_cast<deref_operator_c*>(symbol->exp))
                    STAGE4_ERROR(symbol,
                                 symbol->exp,
                                 "The use of two or more consecutive derefencing operators between a struct variable "
                                 "and its record elem (ex: struct_ref_ref^^.elem) is currently not supported for code "
    ...
    void* visit(deref_expression_c* symbol) {
        ...
        if (this->is_variable_prefix_null()) {
            ...
            if (vartype == search_var_instance_decl_c::external_vt) {
                if (!get_datatype_info_c::is_type_valid(symbol->exp->datatype))
                    ERROR;
    ...
    void* visit(function_invocation_c* symbol) {
        ...
        if (NULL != symbol->formal_param_list)
            parameter_assignment_list = symbol->formal_param_list;
        if (NULL != symbol->nonformal_param_list)
            parameter_assignment_list = symbol->nonformal_param_list;
        // NOTE-> We support the non-standard feature of POUS with no in, out and inout parameters, so this is no longer
        // an internal error!
        ...
        /* flag to cirreclty handle calls to extensible standard functions (i.e. functions with variable number of input
         * parameters) */
        ...
        for (int i = 1; (param_name = fp_iterator.next()) != NULL; i++) {
            if (fp_iterator.is_extensible_param() && (!found_first_extensible_parameter)) {
                ...
                char* tmp = (char*)malloc(
                    32); /* enough space for a call with 10^31 (larger than 2^64) input parameters! */
                if (tmp == NULL)
                    ERROR;
        ...
        if (function_call_param_iterator.next_nf() != NULL)
            ERROR;
    ...
    void* visit(fb_invocation_c* symbol) {
        ...
        if (NULL == function_block_type_name)
            ERROR;
    ...
    void* visit(case_element_list_c* symbol) {
        return print_list(symbol, s4o.indent_spaces + "if ", s4o.indent_spaces + "else if ");
    }
    ...
    void* visit(for_statement_c* symbol) {
        ...
        if (symbol->by_expression == NULL) {
            ...
            integer_oneval.const_value._int64v.set(1);                     // set the stage3 anottation we need
