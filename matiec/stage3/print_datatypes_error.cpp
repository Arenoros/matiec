 ...
 *       BEFORE running this visitor, be sure to CALL the fill_candidate_datatypes_c, and the
 * narrow_candidate_datatypes_c visitors!
 */
...
#include "print_datatypes_error.h"
...
void print_datatypes_error_c::handle_function_invocation(symbol_c* fcall, generic_function_call_t fcall_data) {
    ...
    if (generic_function_call_t::POU_FB == fcall_data.POU_type)
        POU_str = "FB";
    if (generic_function_call_t::POU_function == fcall_data.POU_type)
        POU_str = "function";
    if (NULL == POU_str)
        ERROR;
    ...
    if ((NULL == f_decl) && (generic_function_call_t::POU_FB == fcall_data.POU_type)) {
        ...
        /* I.e., a FB invocation using an undefined FB variable is not possible in the current implementation of
         * stage 2. */
        ERROR;
    }
    if (NULL == f_decl) {
            ...
            while ((param_name = fcp_iterator.next_f()) != NULL) {
                ...
                if (fcp_iterator.search_f(param_name) != param_value) {
                    ...
                    STAGE3_ERROR(0,
                                 param_name,
                                 param_name,
                                 "Duplicate parameter '%s' when invoking %s '%s'",
                                 ((token_c*)param_name)->value,
                                 POU_str,
                                 ((token_c*)fcall_data.function_name)->value);
                ...
                if (NULL == fp_iterator.search(param_name)) {
                    ...
                    STAGE3_ERROR(0,
                                 param_name,
                                 param_name,
                                 "Invalid parameter '%s' when invoking %s '%s'",
                                 ((token_c*)param_name)->value,
                                 POU_str,
                                 ((token_c*)fcall_data.function_name)->value);
                    continue; /* jump to next parameter */
                ...
                if (function_call_param_iterator_c::assign_in == call_param_dir) {
                    if ((function_param_iterator_c::direction_in != param_dir) &&
                        ...
                        STAGE3_ERROR(0,
                                     param_name,
                                     param_name,
                                     "Invalid assignment syntax ':=' used for parameter '%s', when invoking %s '%s'",
                                     ((token_c*)param_name)->value,
                                     POU_str,
                                     ((token_c*)fcall_data.function_name)->value);
                        continue; /* jump to next parameter */
                ...
                if (!get_datatype_info_c::is_type_valid(param_value->datatype)) {
                    ...
                    STAGE3_ERROR(0,
                                 param_value,
                                 param_value,
                                 "Data type incompatibility between parameter '%s' and value being passed, when "
                                 "invoking %s '%s'",
                                 ((token_c*)param_name)->value,
                                 POU_str,
                                 ((token_c*)fcall_data.function_name)->value);
    ...
    if (NULL != fcall_data.nonformal_operand_list) {
        if (f_decl)
            for (int i = 1; (param_value = fcp_iterator.next_nf()) != NULL; i++) {
                ...
                /* This handle_function_invocation() will be called to handle IL function calls, where the first
                 * parameter comes from the previous IL instruction. In this case, the previous IL instruction will be
                 * artifically (and temporarily) added to the begining ot the parameter list so we (in this function)
                 * can handle this situation like all the other function calls. However, a) if NO previous IL function
                 * exists, then we get a fake previous IL function, with no location data (i.e. not found anywhere in
                 * the source code. b) the function call may actually have several prev IL instructions (if several JMP
                 * instructions jump directly to the il function call). In order to handle these situations gracefully,
                 * we first check whether the first parameter is really an IL istruction!
                 */
                il_instruction_c* il_instruction_symbol = dynamic_cast<il_instruction_c*>(param_value);
                if ((NULL != il_instruction_symbol) && (i == 1)) {
                    /* We are in a situation where an IL function call is passed the first parameter, which is actually
                     * the previous IL instruction */
                    /* However, this is really a fake previous il instruction (see visit(il_instruction_c *) )
                     * We will iterate through all the real previous IL instructions, and analyse each of them one by
                     * one */
                    if (il_instruction_symbol->prev_il_instruction.size() == 0) {
                        ...
                        STAGE3_ERROR(0,
                                     fcall,
                                     fcall,
                                     "No available data to pass to first parameter of IL function %s. Missing a "
                                     "previous LD instruction?",
                                     ((token_c*)fcall_data.function_name)->value);
                    }
#if 0
                    ...
                    if (!get_datatype_info_c::is_type_valid(il_instruction_symbol->datatype)) {
                        ...
                        STAGE3_ERROR(0,
                                     fcall,
                                     fcall,
                                     "Data type incompatibility between value in IL 'accumulator' and first parameter "
                                     "of function '%s'",
                                     ((token_c*)fcall_data.function_name)->value);
                    }
#endif
                    if (function_invocation_error)
                        /* when handling a IL function call, and an error is found in the first parameter, then we bug
                         * out and do not print out any more error messages. */
                        return;
                } else {
                    if (!get_datatype_info_c::is_type_valid(param_value->datatype)) {
                        function_invocation_error = true;
                        STAGE3_ERROR(0,
                                     param_value,
                                     param_value,
                                     "Data type incompatibility for value passed in position %d when invoking %s '%s'",
                                     i,
                                     POU_str,
                                     ((token_c*)fcall_data.function_name)->value);
                    }
    ...
    if (NULL == fcall_data.called_function_declaration) {
        ...
        STAGE3_ERROR(0,
                     fcall,
                     fcall,
                     "Unable to resolve which overloaded %s '%s' is being invoked.",
                     POU_str,
                     ((token_c*)fcall_data.function_name)->value);
    }
    ...
    if (function_invocation_error) {
        ...
        STAGE3_ERROR(2,
                     fcall,
                     fcall,
                     "Invalid parameters when invoking %s '%s'",
                     POU_str,
                     ((token_c*)fcall_data.function_name)->value);
    }
...
void* print_datatypes_error_c::handle_implicit_il_fb_invocation(const char* param_name,
                                                                symbol_c* il_operator,
    ...
    if (fake_prev_il_instruction->prev_il_instruction.empty()) {
        STAGE3_ERROR(0,
                     il_operator,
                     il_operand,
                     "FB invocation operator '%s' must be preceded by a 'LD' (or equivalent) operator.",
                     param_name);
        return NULL;
    }
    ...
    if (NULL == fp_iterator.search(param_name)) {
        ...
        STAGE3_ERROR(0,
                     il_operator,
                     il_operand,
                     "FB called by '%s' operator does not have a parameter named '%s'",
                     param_name,
                     param_name);
        return NULL;
    }
    if (!are_all_datatypes_equal(fake_prev_il_instruction->prev_il_instruction)) {
        STAGE3_ERROR(0,
                     il_operator,
                     il_operand,
                     "Data type incompatibility between parameter '%s' and value being passed.",
                     param_name);
        return NULL;
    }
    ...
    if (!get_datatype_info_c::is_type_valid(il_operand->datatype)) {
        ...
        STAGE3_ERROR(0,
                     il_operator,
                     il_operator,
                     "Invalid FB call: Datatype incompatibility between the FB's '%s' parameter and value being "
                     "passed, or paramater '%s' is not a 'VAR_INPUT' parameter.",
                     param_name,
                     param_name);
        return NULL;
        // 		}
...
void* print_datatypes_error_c::visit(integer_literal_c* symbol) {
    if (symbol->candidate_datatypes.size() == 0) {
        STAGE3_ERROR(0,
                     symbol,
                     symbol,
                     "Numerical value exceeds range for %s data type.",
                     get_datatype_info_c::get_id_str(symbol->type));
    } else if (!get_datatype_info_c::is_type_valid(symbol->datatype)) {
        STAGE3_ERROR(4, symbol, symbol, "ANY_INT data type not valid in this location.");
    }
    return NULL;
...
void* print_datatypes_error_c::visit(boolean_literal_c* symbol) {
    if (symbol->candidate_datatypes.size() == 0) {
        STAGE3_ERROR(0,
                     symbol,
                     symbol,
                     "Value is not valid for %s data type.",
                     get_datatype_info_c::get_id_str(symbol->type));
    } else if (!get_datatype_info_c::is_type_valid(symbol->datatype)) {
...
void* print_datatypes_error_c::visit(structure_element_initialization_c* symbol) {
    ...
    if (!get_datatype_info_c::is_type_valid(symbol->datatype))
        STAGE3_ERROR(0,
                     symbol,
                     symbol,
                     "Initialization element identifier (%s) is not declared in referenced structure/FB scope, or is "
                     "set to value of incompatible datatype.",
...
void* print_datatypes_error_c::visit(subscript_list_c* symbol) {
    for (int i = 0; i < symbol->size(); i++) {
        ...
        if ((start_error_count == error_count) &&
            (!get_datatype_info_c::is_type_valid(symbol->get_element(i)->datatype)))
...
void* print_datatypes_error_c::visit(structured_variable_c* symbol) {
    ...
    if (symbol->candidate_datatypes.size() == 0)
        STAGE3_ERROR(0,
                     symbol,
                     symbol,
                     "Undeclared structured (or FB) variable, or non-existant field (variable) in structure (FB).");
    return NULL;
}
...
void* print_datatypes_error_c::visit(located_var_decl_c* symbol) {
    ...
    /* It does not make sense to call symbol->location->accept(*this). The check is done right here if the following
     * if() */
    // symbol->location->accept(*this);
    if ((get_datatype_info_c::is_type_valid(symbol->located_var_spec_init->datatype)) &&
        (!get_datatype_info_c::is_type_valid(symbol->location->datatype)))
...
void* print_datatypes_error_c::visit(function_declaration_c* symbol) {
    ...
    if (debug)
        printf("Print error data types list in body of function %s\n",
...
void* print_datatypes_error_c::visit(function_block_declaration_c* symbol) {
    ...
    if (debug)
        printf("Print error data types list in body of FB %s\n", ((token_c*)(symbol->fblock_name))->value);
    il_parenthesis_level = 0;
    il_error = false;
...
void* print_datatypes_error_c::visit(program_declaration_c* symbol) {
    ...
    symbol->var_declarations->accept(*this);
    if (debug)
        printf("Print error data types list in body of program %s\n", ((token_c*)(symbol->program_type_name))->value);
    il_parenthesis_level = 0;
    il_error = false;
...
#endif
        /* the print error algorithm will need access to the intersected candidate_datatype lists of all
...
void* print_datatypes_error_c::visit(il_simple_operation_c* symbol) {
    ...
    if (NULL != il_operand)
        symbol->il_operand->accept(*this); /* recursive call to see whether data types are compatible */
...
void* print_datatypes_error_c::visit(il_expression_c* symbol) {
    ...
    /* Now handle the operation (il_expr_operator) that will use the result coming from the parenthesised IL list (i.e.
     * simple_instr_list) */
    il_operand =
...
void* print_datatypes_error_c::visit(il_fb_call_c* symbol) {
    ...
    generic_function_call_t fcall_param = {
        ...
        /* fcall_param.candidate_functions         = */ candidate_functions, /* will not be used, but must provide a
                                                                                reference to be able to compile */
        /* fcall_param.called_function_declaration = */ symbol->called_fb_declaration,
        /* fcall_param.extensible_param_count      = */ extensible_param_count /* will not be used, but must provide a
                                                                                  reference to be able to compile */
    };
...
void* print_datatypes_error_c::visit(il_simple_instruction_c* symbol) {
  ...
  tmp_prev_il_instruction.prev_il_instruction = symbol->prev_il_instruction;
    ...
    /* the print error algorithm will need access to the intersected candidate_datatype lists of all
     * prev_il_instructions, as well as the list of the prev_il_instructions. Instead of creating two 'global' (within
     * the class) variables, we create a single il_instruction_c variable (fake_prev_il_instruction), and shove that
     * data into this single variable.
     */
    tmp_prev_il_instruction.prev_il_instruction = symbol->prev_il_instruction;
    intersect_prev_candidate_datatype_lists(&tmp_prev_il_instruction);
    if (are_all_datatypes_equal(symbol->prev_il_instruction))
...
/* B 2.2 Operators */
/*******************/
void* print_datatypes_error_c::print_binary_operator_errors(const char* il_operator,
                                                            symbol_c* symbol,
...
void* print_datatypes_error_c::visit(LD_operator_c* symbol) {
    return print_binary_operator_errors("LD", symbol);
}  // I believe it will never emit messages (b) and (c)!!
void* print_datatypes_error_c::visit(LDN_operator_c* symbol) {
    return print_binary_operator_errors("LDN", symbol);
}  // I believe it will never emit message (c)
void* print_datatypes_error_c::visit(ST_operator_c* symbol) {
    return print_binary_operator_errors("ST", symbol);
}  // I believe it will never emit message (c)
void* print_datatypes_error_c::visit(STN_operator_c* symbol) {
    return print_binary_operator_errors("STN", symbol);
}  // I believe it will never emit message (c)
...
void* print_datatypes_error_c::visit(S_operator_c* symbol) {
    if (NULL != symbol->called_fb_declaration) /* FB call semantics */
        return handle_implicit_il_fb_invocation("S", symbol, symbol->called_fb_declaration);
    else /* Reset   semantics */
        return print_binary_operator_errors("S", symbol);
}

void* print_datatypes_error_c::visit(R_operator_c* symbol) {
    if (NULL != symbol->called_fb_declaration) /* FB call semantics */
        return handle_implicit_il_fb_invocation("R", symbol, symbol->called_fb_declaration);
    else /* Reset   semantics */
        return print_binary_operator_errors("R", symbol);
}

void* print_datatypes_error_c::visit(S1_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("S1", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(R1_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("R1", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(CLK_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("CLK", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(CU_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("CU", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(CD_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("CD", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(PV_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("PV", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(IN_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("IN", symbol, symbol->called_fb_declaration);
}
void* print_datatypes_error_c::visit(PT_operator_c* symbol) {
    return handle_implicit_il_fb_invocation("PT", symbol, symbol->called_fb_declaration);
}

void* print_datatypes_error_c::visit(AND_operator_c* symbol) {
    return print_binary_operator_errors("AND", symbol);
}
void* print_datatypes_error_c::visit(OR_operator_c* symbol) {
    return print_binary_operator_errors("OR", symbol);
}
void* print_datatypes_error_c::visit(XOR_operator_c* symbol) {
    return print_binary_operator_errors("XOR", symbol);
}
void* print_datatypes_error_c::visit(ANDN_operator_c* symbol) {
    return print_binary_operator_errors("ANDN", symbol);
}
void* print_datatypes_error_c::visit(ORN_operator_c* symbol) {
    return print_binary_operator_errors("ORN", symbol);
}
void* print_datatypes_error_c::visit(XORN_operator_c* symbol) {
    return print_binary_operator_errors("XORN", symbol);
}
void* print_datatypes_error_c::visit(ADD_operator_c* symbol) {
    return print_binary_operator_errors("ADD", symbol, symbol->deprecated_operation);
}
void* print_datatypes_error_c::visit(SUB_operator_c* symbol) {
    return print_binary_operator_errors("SUB", symbol, symbol->deprecated_operation);
}
void* print_datatypes_error_c::visit(MUL_operator_c* symbol) {
    return print_binary_operator_errors("MUL", symbol, symbol->deprecated_operation);
}
void* print_datatypes_error_c::visit(DIV_operator_c* symbol) {
    return print_binary_operator_errors("DIV", symbol, symbol->deprecated_operation);
}
void* print_datatypes_error_c::visit(MOD_operator_c* symbol) {
    return print_binary_operator_errors("MOD", symbol);
}

void* print_datatypes_error_c::visit(GT_operator_c* symbol) {
    return print_binary_operator_errors("GT", symbol);
}
void* print_datatypes_error_c::visit(GE_operator_c* symbol) {
    return print_binary_operator_errors("GE", symbol);
}
void* print_datatypes_error_c::visit(EQ_operator_c* symbol) {
    return print_binary_operator_errors("EQ", symbol);
}
void* print_datatypes_error_c::visit(LT_operator_c* symbol) {
    return print_binary_operator_errors("LT", symbol);
}
void* print_datatypes_error_c::visit(LE_operator_c* symbol) {
    return print_binary_operator_errors("LE", symbol);
}
void* print_datatypes_error_c::visit(NE_operator_c* symbol) {
    return print_binary_operator_errors("NE", symbol);
}
...
void* print_datatypes_error_c::handle_conditional_flow_control_IL_instruction(symbol_c* symbol, const char* oper) {
    if (!get_datatype_info_c::is_type_valid(symbol->datatype))
        STAGE3_ERROR(0,
                     symbol,
                     symbol,
                     "%s operator must be preceded by an IL instruction producing a BOOL value.",
                     oper);
    return NULL;
}
...
void* print_datatypes_error_c::print_binary_expression_errors(const char* operation,
                                                              symbol_c* symbol,
                                                              symbol_c* l_expr,
                                                              symbol_c* r_expr,
                                                              bool deprecated_operation) {
    l_expr->accept(*this);
    r_expr->accept(*this);
    if ((symbol->candidate_datatypes.size() == 0) && (l_expr->candidate_datatypes.size() > 0) &&
...
void* print_datatypes_error_c::visit(or_expression_c* symbol) {
    return print_binary_expression_errors("OR", symbol, symbol->l_exp, symbol->r_exp);
}
void* print_datatypes_error_c::visit(xor_expression_c* symbol) {
    return print_binary_expression_errors("XOR", symbol, symbol->l_exp, symbol->r_exp);
}
void* print_datatypes_error_c::visit(and_expression_c* symbol) {
    return print_binary_expression_errors("AND", symbol, symbol->l_exp, symbol->r_exp);
}
void* print_datatypes_error_c::visit(equ_expression_c* symbol) {
    return print_binary_expression_errors("=", symbol, symbol->l_exp, symbol->r_exp);
}
...
void* print_datatypes_error_c::visit(add_expression_c* symbol) {
    return print_binary_expression_errors("+", symbol, symbol->l_exp, symbol->r_exp, symbol->deprecated_operation);
}
...
void* print_datatypes_error_c::visit(mod_expression_c* symbol) {
    return print_binary_expression_errors("MOD", symbol, symbol->l_exp, symbol->r_exp);
}
...
void* print_datatypes_error_c::visit(assignment_statement_c* symbol) {
    ...
    if ((!get_datatype_info_c::is_type_valid(symbol->l_exp->datatype)) &&
        ...
        (symbol->l_exp->candidate_datatypes.size() > 0) && (symbol->r_exp->candidate_datatypes.size() > 0))
...
void* print_datatypes_error_c::visit(if_statement_c* symbol) {
    ...
    if ((!get_datatype_info_c::is_type_valid(symbol->expression->datatype)) &&
        ...
        STAGE3_ERROR(0,
                     symbol->expression,
                     symbol->expression,
                     "Invalid data type for 'IF' condition (should be BOOL).");
    }
...
void* print_datatypes_error_c::visit(elseif_statement_c* symbol) {
    ...
    if ((!get_datatype_info_c::is_type_valid(symbol->expression->datatype)) &&
        ...
        STAGE3_ERROR(0,
                     symbol->expression,
                     symbol->expression,
                     "Invalid data type for 'ELSIF' condition (should be BOOL).");
    }
    if (NULL != symbol->statement_list)
        symbol->statement_list->accept(*this);
    return NULL;
...
void* print_datatypes_error_c::visit(for_statement_c* symbol) {
    ...
    /* BY expression */
    if ((NULL != symbol->by_expression) && (!get_datatype_info_c::is_type_valid(symbol->by_expression->datatype)) &&
        (symbol->end_expression->candidate_datatypes.size() > 0)) {
