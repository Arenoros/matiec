...
private:
    ...
    static std::set<symbol_c*>
        checked_decl;  // A set with all the declarations that have already been checked, so we don't recheck it again!
...
public:
    ...
    void check_global_decl(symbol_c* p_decl) {
        if (NULL == current_pou_decl)
            ERROR;

        if (checked_decl.find(p_decl) != checked_decl.end())
            ...
            if (fpi.param_direction() == function_param_iterator_c::direction_extref) {
                ...
                if (NULL == glo_decl) {
                    STAGE3_ERROR(0,
                                 ext_decl,
                                 ext_decl,
                                 "Declaration error. The external variable does not match with any global variable.");
                    continue;
                }
                ...
                if ((glo_opt == search_var_instance_decl_c::constant_opt) &&
                    (ext_opt != search_var_instance_decl_c::constant_opt))
                    STAGE3_ERROR(0,
                                 glo_decl,
                                 glo_decl,
                                 "Declaration error. The external variable must be declared as constant, as it maps to "
    ...
    void* visit(fb_spec_init_c* symbol) {
        auto iter = function_block_type_symtable.find(symbol->function_block_type_name);
    ...
    void* visit(function_block_declaration_c* symbol) {
        ...
        /* The following two lines of code are only valid for v3 of IEC 61131-3, that allows VAR_GLOBAL declarations
         * inside FBs! current_pou_decl = symbol; current_resource_decl = NULL;
         */
        /* check if any FB declared as a VAR has any incompatible VAR_EXTERNAL declarations */
        if (NULL != symbol->var_declarations)
            symbol->var_declarations->accept(
                *this);  // This will eventually call the visit(fb_spec_init_c *) method, if any FB are declared here!
        return NULL;
    }
    ...
    void* visit(program_declaration_c* symbol) {
        check_global_decl(symbol);
        /* The following two lines of code are only valid for v3 of IEC 61131-3, that allows VAR_GLOBAL declarations
         * inside PROGRAMs! current_pou_decl = symbol; current_resource_decl = NULL;
         */
        /* check if any FB declared as a VAR has any incompatible VAR_EXTERNAL declarations */
        if (NULL != symbol->var_declarations)
            symbol->var_declarations->accept(
...
void* declaration_check_c::visit(function_block_declaration_c* symbol) {
    return NULL;
}  // We only check the declarations that are used to instantiate variables. This is done in the configuration!
...
void* declaration_check_c::visit(resource_declaration_c* symbol) {
    // check if any FB instantiated inside this resource (in a VAR_GLOBAL) has any VAR_EXTERNAL declarations
    // incompatible with the configuration's VAR_GLOBALs
...
void* declaration_check_c::visit(program_configuration_c* symbol) {
    ...
    auto iter_p = program_type_symtable.find(symbol->program_type_name);
    auto iter_f = function_block_type_symtable.find(symbol->program_type_name);

    if (iter_p != program_type_symtable.end())
        p_decl = iter_p->second;
    if (iter_f != function_block_type_symtable.end())
        p_decl = iter_f->second;
    ...
    if ((iter_f == function_block_type_symtable.end()) && (iter_p == program_type_symtable.end()))
        ERROR;  // Should never occur! stage1_2 guarantees that we are sure to find a declaration in FB or Program
                // symtable.
