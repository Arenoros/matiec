...
/*
 *  Choose, from the list of all the possible datatypes each expression may take, the single datatype that it will in
 * fact take. The resulting (chosen) datatype, will be stored in the symbol_c.datatype variable, leaving the candidate
 * datatype list untouched!
 *
 ...
 *  then a datatype error has been found, and the datatype is either left at NULL, or set to a pointer of an
 * invalid_type_name_c object!
 */

#include "narrow_candidate_datatypes.h"
...
static void set_datatype(symbol_c* datatype, symbol_c* symbol) {
    ...
    if ((NULL == datatype) && (NULL != symbol->datatype))
        return;
    if ((NULL == datatype) && (NULL == symbol->datatype))
        return;
...
/* Only set the symbol's desired datatype to 'datatype' if that datatype is in the candidate_datatype list */
// NOTE: This function is virtual! The forced_narrow_candidate_datatypes_c has a slightly different version of this
// fuinction!!
...
bool narrow_candidate_datatypes_c::is_widening_compatible(const struct widen_entry widen_table[],
                                                          symbol_c* left_type,
                                                          symbol_c* right_type,
                                                          symbol_c* result_type,
                                                          bool* deprecated_status) {
    /* NOTE: According to our algorithm, left_type and right_type should never by NULL (if they are, we have an internal
     * compiler error! However, result_type may be NULL if the code has a data type semantic error!
     */
...
void narrow_candidate_datatypes_c::narrow_nonformal_call(symbol_c* f_call, symbol_c* f_decl, int* ext_parm_count) {
    ...
    while ((call_param_value = fcp_iterator.next_nf()) != NULL) {
        ...
        do {
            ...
            /* This error should have been caught in fill_candidate_datatypes_c, but may occur here again when we handle
             * FB invocations! In this case, we carry on analysing the code in order to be able to provide relevant
             * error messages for that code too!
             */
            if (param_name == NULL)
                break;
        ...
        if ((NULL != param_name) && (NULL == desired_datatype))
            ERROR;
        if ((NULL == param_name) && (NULL != desired_datatype))
            ERROR;

        /* NOTE: When we are handling a nonformal function call made from IL, the first parameter is the 'default' or
         * 'current' il value. However, a pointer to a copy of the prev_il_instruction is pre-pended into the operand
         * list, so the call call_param_value->accept(*this); may actually be calling an object of the base symbol_c .
         */
...
void narrow_candidate_datatypes_c::narrow_function_invocation(symbol_c* fcall, generic_function_call_t fcall_data) {
    ...
    if (NULL != fcall_data.nonformal_operand_list)
        narrow_nonformal_call(fcall, fcall_data.called_function_declaration, &(fcall_data.extensible_param_count));
    if (NULL != fcall_data.formal_operand_list)
        narrow_formal_call(fcall, fcall_data.called_function_declaration, &(fcall_data.extensible_param_count));
 ...
 */
void* narrow_candidate_datatypes_c::narrow_implicit_il_fb_call(symbol_c* il_instruction,
    ...
    if (NULL == fb_decl) {
        ...
        /* The invalid FB invocation will be caught in the print_datatypes_error_c by analysing NULL value in
         * il_operand->datatype! */
     ...
     * would be changed to the IL instruction coming before the current prev_il_instruction! => things would get all
     * messed up!) The easiest way to work around this is to simply use a new object, and copy the relevant details to
     * that object!
     */
    ...
    // SYM_REF4(il_fb_call_c, il_call_operator, fb_name, il_operand_list, il_param_list, symbol_c
    // *called_fb_declaration)
     ...
     */
    if ((NULL == il_instruction->datatype) ||
        (get_datatype_info_c::is_type_equal(param_value.datatype, il_instruction->datatype))) {
...
void* narrow_candidate_datatypes_c::visit(derived_datatype_identifier_c* symbol) {
    // If this symbol was used (for example) in an ARRAY [1..2] OF <derived_datatype_identifier_c> (i.e. a datatype in
    // an array) then the symbol->datatype of this derived_datatype_identifier_c has not yet been set by the previous
    // visit() method! We therefore set the datatype ourselves!
...
void *narrow_candidate_datatypes_c::visit(         poutype_identifier_c *symbol) {
    // If this symbol was used (for example) in an ARRAY [1..2] OF <derived_datatype_identifier_c> (i.e. a datatype in
an array)
    // then the symbol->datatype of this derived_datatype_identifier_c has not yet been set by the previous visit()
method!
...
void* narrow_candidate_datatypes_c::visit(time_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(bool_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(sint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(int_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(dint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(lint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(usint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(uint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(udint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(ulint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(real_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(lreal_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(date_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(tod_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(dt_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(byte_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(word_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(dword_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(lword_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(string_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(wstring_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}

void* narrow_candidate_datatypes_c::visit(safetime_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safebool_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safesint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safeint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safedint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safelint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safeusint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safeuint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safeudint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safeulint_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safereal_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safelreal_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safedate_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safetod_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safedt_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safebyte_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safeword_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safedword_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safelword_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safestring_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
void* narrow_candidate_datatypes_c::visit(safewstring_type_name_c* symbol) {
    symbol->datatype = search_base_type_c::get_basetype_decl(symbol);
    return NULL;
}
...
void* narrow_candidate_datatypes_c::narrow_spec_init(symbol_c* symbol, symbol_c* type_decl, symbol_c* init_value) {
    // If we are handling an anonymous datatype (i.e. a datatype implicitly declared inside a VAR ... END_VAR
    // declaration) then the symbol->datatype has not yet been set by the previous visit(type_decl) method, because it
    // does not exist! So we set the datatype ourselves!
    if ((NULL == symbol->datatype) && (symbol->candidate_datatypes.size() == 1))
        symbol->datatype = symbol->candidate_datatypes[0];
...
void* narrow_candidate_datatypes_c::visit(simple_type_declaration_c* symbol) {
    return narrow_type_decl(symbol, symbol->simple_type_name, symbol->simple_spec_init);
}

/* simple_specification ASSIGN constant */
// SYM_REF2(simple_spec_init_c, simple_specification, constant)
void* narrow_candidate_datatypes_c::visit(simple_spec_init_c* symbol) {
    return narrow_spec_init(symbol, symbol->simple_specification, symbol->constant);
}

/*  subrange_type_name ':' subrange_spec_init */
// SYM_REF2(subrange_type_declaration_c, subrange_type_name, subrange_spec_init)
void* narrow_candidate_datatypes_c::visit(subrange_type_declaration_c* symbol) {
    return narrow_type_decl(symbol, symbol->subrange_type_name, symbol->subrange_spec_init);
}

/* subrange_specification ASSIGN signed_integer */
// SYM_REF2(subrange_spec_init_c, subrange_specification, signed_integer)
void* narrow_candidate_datatypes_c::visit(subrange_spec_init_c* symbol) {
    return narrow_spec_init(symbol, symbol->subrange_specification, symbol->signed_integer);
}
...
void* narrow_candidate_datatypes_c::visit(enumerated_type_declaration_c* symbol) {
    return narrow_type_decl(symbol, symbol->enumerated_type_name, symbol->enumerated_spec_init);
}
...
void* narrow_candidate_datatypes_c::visit(enumerated_spec_init_c* symbol) {
    return narrow_spec_init(symbol, symbol->enumerated_specification, symbol->enumerated_value);
}
...
void* narrow_candidate_datatypes_c::visit(enumerated_value_list_c* symbol) {
    ...
    for (int i = 0; i < symbol->size(); i++)
        set_datatype(symbol->datatype, symbol->get_element(i));
    // for(int i = 0; i < symbol->size(); i++) if (NULL == symbol->get_element(i)->datatype) ERROR; // Comented out->
    // Reserve this check for the print_datatypes_error_c ???
    return NULL;
}
...
void* narrow_candidate_datatypes_c::visit(array_type_declaration_c* symbol) {
    return narrow_type_decl(symbol, symbol->identifier, symbol->array_spec_init);
}

/* array_specification [ASSIGN array_initialization} */
/* array_initialization may be NULL ! */
// SYM_REF2(array_spec_init_c, array_specification, array_initialization)
void* narrow_candidate_datatypes_c::visit(array_spec_init_c* symbol) {
    return narrow_spec_init(symbol, symbol->array_specification, symbol->array_initialization);
}
...
void* narrow_candidate_datatypes_c::visit(structure_type_declaration_c* symbol) {
    return narrow_type_decl(symbol, symbol->structure_type_name, symbol->structure_specification);
}

/* structure_type_name ASSIGN structure_initialization */
/* structure_initialization may be NULL ! */
// SYM_REF2(initialized_structure_c, structure_type_name, structure_initialization)
void* narrow_candidate_datatypes_c::visit(initialized_structure_c* symbol) {
    return narrow_spec_init(symbol, symbol->structure_type_name, symbol->structure_initialization);
}
...
void* narrow_candidate_datatypes_c::visit(structure_element_initialization_list_c* symbol) {
    ...
    if (get_datatype_info_c::is_type_valid(type)) {
        ...
        for (int k = 0; k < symbol->size(); k++) {
            structure_element_initialization_c* struct_elem = (structure_element_initialization_c*)symbol->get_element(
                k);
            symbol_c* type = NULL;
            if (struct_decl == NULL) {
            ...
            // if (struct_elem->datatype == NULL) ERROR; // should never occur. Already checked in
...
void* narrow_candidate_datatypes_c::visit(structure_element_initialization_c* symbol) {
    set_datatype(symbol->datatype, symbol->value);
...
/* structure_initialization may be NULL ! */
// SYM_REF2(initialized_structure_c, structure_type_name, structure_initialization)
void* narrow_candidate_datatypes_c::visit(fb_spec_init_c* symbol) {
    return narrow_spec_init(symbol, symbol->function_block_type_name, symbol->structure_initialization);
}
...
void* narrow_candidate_datatypes_c::visit(ref_spec_init_c* symbol) {
    return narrow_spec_init(symbol, symbol->ref_spec, symbol->ref_initialization);
}

/* identifier ':' ref_spec_init */
// SYM_REF2(ref_type_decl_c, ref_type_name, ref_spec_init)
void* narrow_candidate_datatypes_c::visit(ref_type_decl_c* symbol) {
    return narrow_type_decl(symbol, symbol->ref_type_name, symbol->ref_spec_init);
}
...
void* narrow_candidate_datatypes_c::visit(array_variable_c* symbol) {
    ...
    /* Please read the comments in the array_variable_c and structured_variable_c visitors in the
     * fill_candidate_datatypes.cc file! */
    if (symbol->subscripted_variable->candidate_datatypes.size() == 1)
        symbol->subscripted_variable->datatype = symbol->subscripted_variable
                                                     ->candidate_datatypes[0];  // set the datatype
...
void* narrow_candidate_datatypes_c::visit(var1_init_decl_c* symbol) {
    return narrow_var_declaration(symbol->spec_init);
}
void* narrow_candidate_datatypes_c::visit(array_var_init_decl_c* symbol) {
    return narrow_var_declaration(symbol->array_spec_init);
}
void* narrow_candidate_datatypes_c::visit(structured_var_init_decl_c* symbol) {
    return narrow_var_declaration(symbol->initialized_structure);
}
void* narrow_candidate_datatypes_c::visit(fb_name_decl_c* symbol) {
    return narrow_var_declaration(symbol->fb_spec_init);
}
void* narrow_candidate_datatypes_c::visit(array_var_declaration_c* symbol) {
    return narrow_var_declaration(symbol->array_specification);
}
void* narrow_candidate_datatypes_c::visit(structured_var_declaration_c* symbol) {
    return narrow_var_declaration(symbol->structure_type_name);
}
void* narrow_candidate_datatypes_c::visit(external_declaration_c* symbol) {
    return narrow_var_declaration(symbol->specification);
}
void* narrow_candidate_datatypes_c::visit(global_var_decl_c* symbol) {
    return narrow_var_declaration(symbol->type_specification);
}
void* narrow_candidate_datatypes_c::visit(incompl_located_var_decl_c* symbol) {
    return narrow_var_declaration(symbol->var_spec);
}
// void *narrow_candidate_datatypes_c::visit(single_byte_string_var_declaration_c *symbol) {return
// handle_var_declaration(symbol->single_byte_string_spec);} void
// *narrow_candidate_datatypes_c::visit(double_byte_string_var_declaration_c *symbol) {return
...
void* narrow_candidate_datatypes_c::visit(function_declaration_c* symbol) {
    ...
    if (debug)
        printf("Narrowing candidate data types list in body of function %s\n",
...
void* narrow_candidate_datatypes_c::visit(function_block_declaration_c* symbol) {
    ...
    if (debug)
        printf("Narrowing candidate data types list in body of FB %s\n", ((token_c*)(symbol->fblock_name))->value);
...
void* narrow_candidate_datatypes_c::visit(program_declaration_c* symbol) {
    ...
    symbol->var_declarations->accept(*this);
    if (debug)
        printf("Narrowing candidate data types list in body of program %s\n",
...
void* narrow_candidate_datatypes_c::visit(transition_condition_c* symbol) {
    // We can safely ask for a BOOL type, as even if the result is a SAFEBOOL, in that case it will aslo include BOOL as
    // a possible datatype.
...
void* narrow_candidate_datatypes_c::visit(configuration_declaration_c* symbol) {
    if (debug)
        printf("Narrowing candidate data types list in configuration %s\n",
...
void* narrow_candidate_datatypes_c::visit(resource_declaration_c* symbol) {
    if (debug)
        printf("Narrowing candidate data types list in resource %s\n", ((token_c*)(symbol->resource_name))->value);
...
void* narrow_candidate_datatypes_c::visit(il_instruction_c* symbol) {
    if (NULL == symbol->il_instruction) {
        /* this empty/null il_instruction cannot generate the desired datatype. We pass on the request to the previous
         * il instruction. */
        set_datatype_in_prev_il_instructions(symbol->datatype, symbol);
    } else {
        il_instruction_c tmp_prev_il_instruction(NULL, NULL);
        /* the narrow algorithm will need access to the intersected candidate_datatype lists of all
         * prev_il_instructions, as well as the list of the prev_il_instructions. Instead of creating two 'global'
         * (within the class) variables, we create a single il_instruction_c variable (fake_prev_il_instruction), and
         * shove that data into this single variable.
         */
        ...
        /* Tell the il_instruction the datatype that it must generate - this was chosen by the next il_instruction
...
void* narrow_candidate_datatypes_c::visit(il_simple_operation_c* symbol) {
    /* Tell the il_simple_operator the datatype that it must generate - this was chosen by the next il_instruction (we
     * iterate backwards!) */
...
void* narrow_candidate_datatypes_c::visit(il_function_call_c* symbol) {
     ...
     * symbol to the il_operand_list, and remove it after calling handle_function_call(). However, since
     * handle_function_call() will be recursively calling all parameter, and we don't want to do that for the
     * prev_il_instruction (since it has already been visited by the fill_candidate_datatypes_c) we create a new ____
     * symbol_c ____ object, and copy the relevant info to/from that object before/after the call to
...
void* narrow_candidate_datatypes_c::visit(il_expression_c* symbol) {
    /* first handle the operation (il_expr_operator) that will use the result coming from the parenthesised IL list
     * (i.e. simple_instr_list) */
    symbol->il_expr_operator->datatype = symbol->datatype;
    il_operand =
        symbol->simple_instr_list; /* This is not a bug! The parenthesised expression will be used as the operator! */
    ...
    il_instruction_c* save_fake_prev_il_instruction =
        fake_prev_il_instruction; /*this is not really necessary, but lets play it safe */
     ...
     * simple_instr_list will be the equivalent and artificial 'LD <il_operand>' IL instruction. Just to be consistent,
...
void* narrow_candidate_datatypes_c::visit(il_fb_call_c* symbol) {
    ...
    if (NULL == fb_decl)
        ERROR;
    if (NULL != symbol->il_operand_list)
        narrow_nonformal_call(symbol, fb_decl);
    if (NULL != symbol->il_param_list)
        narrow_formal_call(symbol, fb_decl);
...
void* narrow_candidate_datatypes_c::visit(simple_instr_list_c* symbol) {
    if (symbol->size() > 0)
        symbol->get_element(symbol->size() - 1)->datatype = symbol->datatype;
...
void* narrow_candidate_datatypes_c::visit(il_simple_instruction_c* symbol) {
    ...
    il_instruction_c tmp_prev_il_instruction(NULL, NULL);
    /* the narrow algorithm will need access to the intersected candidate_datatype lists of all prev_il_instructions, as
     * well as the list of the prev_il_instructions. Instead of creating two 'global' (within the class) variables, we
     * create a single il_instruction_c variable (fake_prev_il_instruction), and shove that data into this single
     * variable.
     */
...
void* narrow_candidate_datatypes_c::set_il_operand_datatype(symbol_c* il_operand, symbol_c* datatype) {
    if (NULL == il_operand)
        return NULL; /* if no IL operand => error in the source code!! */
    ...
    if ((NULL != il_operand->datatype) && (NULL != datatype)) {
        ...
        if ((!get_datatype_info_c::is_type_valid(datatype)) &&
            ((!get_datatype_info_c::is_type_valid(il_operand->datatype))))
            return NULL;
        ;
    ...
    /* Even if we are not able to determine the il_operand's datatype ('datatype' is NULL), we still visit it
     * recursively, to give a chance of any complex expressions embedded in the il_operand (e.g. expressions inside
     * array subscripts!) to be narrowed too.
     */
    il_operand->accept(*this);
    return NULL;
}

void* narrow_candidate_datatypes_c::narrow_binary_operator(const struct widen_entry widen_table[],
                                                           symbol_c* symbol,
     ...
     */
    if (NULL !=
        symbol->datatype) {  // next IL instructions were able to determine the datatype this instruction should produce
        for (unsigned int i = 0; i < fake_prev_il_instruction->candidate_datatypes.size(); i++) {
            for (unsigned int j = 0; j < il_operand->candidate_datatypes.size(); j++) {
                ...
                if (is_widening_compatible(widen_table,
                     ...
                     * If we try to set more than one distinct datatype on the same symbol, then the datatype will be
                     * set to an invalid_datatype, which is NOT what we want!
                     */
                    return NULL;
                }
            }
...
void* narrow_candidate_datatypes_c::narrow_conditional_operator(symbol_c* symbol) {
     ...
     */
    // I (mario) am confident the fill/narrow algorithms are working correctly, so for now we can disable the
    // assertions!
     ...
     *       we must still provide a bool_type_name_c datatype (if possible, i.e. if it exists in the candidate datatype
     * list). If it is not possible, we set it to NULL
     *
    ...
    if (NULL == symbol->datatype)
        set_datatype(&get_datatype_info_c::bool_type_name /* datatype*/, symbol /* symbol */);
    if (NULL == symbol->datatype)
        ERROR;  // the BOOL is not on the candidate_datatypes! Strange... Probably a bug in fill_candidate_datatype_c
...
void* narrow_candidate_datatypes_c::narrow_S_and_R_operator(symbol_c* symbol,
                                                            const char* param_name,
                                                            symbol_c* called_fb_declaration) {
...
void* narrow_candidate_datatypes_c::narrow_store_operator(symbol_c* symbol) {
    if (symbol->candidate_datatypes.size() == 1) {
        ...
        /* set the desired datatype of the previous il instruction */
        set_datatype_in_prev_il_instructions(symbol->datatype, fake_prev_il_instruction);
        /* In the case of the ST operator, we must set the datatype of the il_instruction_c object that points to this
         * ST_operator_c ourselves, since the following il_instruction_c objects have not done it, as is normal/standard
...
void* narrow_candidate_datatypes_c::visit(LD_operator_c* symbol) {
    return set_il_operand_datatype(il_operand, symbol->datatype);
}
...
void* narrow_candidate_datatypes_c::visit(ST_operator_c* symbol) {
    return narrow_store_operator(symbol);
}
...
void* narrow_candidate_datatypes_c::visit(S_operator_c* symbol) {
    return narrow_S_and_R_operator(symbol, "S", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(R_operator_c* symbol) {
    return narrow_S_and_R_operator(symbol, "R", symbol->called_fb_declaration);
}

void* narrow_candidate_datatypes_c::visit(S1_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "S1", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(R1_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "R1", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(CLK_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "CLK", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(CU_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "CU", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(CD_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "CD", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(PV_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "PV", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(IN_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "IN", symbol->called_fb_declaration);
}
void* narrow_candidate_datatypes_c::visit(PT_operator_c* symbol) {
    return narrow_implicit_il_fb_call(symbol, "PT", symbol->called_fb_declaration);
}

void* narrow_candidate_datatypes_c::visit(AND_operator_c* symbol) {
    return narrow_binary_operator(widen_AND_table, symbol);
}
void* narrow_candidate_datatypes_c::visit(OR_operator_c* symbol) {
    return narrow_binary_operator(widen_OR_table, symbol);
}
void* narrow_candidate_datatypes_c::visit(XOR_operator_c* symbol) {
    return narrow_binary_operator(widen_XOR_table, symbol);
}
...
void* narrow_candidate_datatypes_c::visit(ADD_operator_c* symbol) {
    return narrow_binary_operator(widen_ADD_table, symbol, &(symbol->deprecated_operation));
}
void* narrow_candidate_datatypes_c::visit(SUB_operator_c* symbol) {
    return narrow_binary_operator(widen_SUB_table, symbol, &(symbol->deprecated_operation));
}
void* narrow_candidate_datatypes_c::visit(MUL_operator_c* symbol) {
    return narrow_binary_operator(widen_MUL_table, symbol, &(symbol->deprecated_operation));
}
void* narrow_candidate_datatypes_c::visit(DIV_operator_c* symbol) {
    return narrow_binary_operator(widen_DIV_table, symbol, &(symbol->deprecated_operation));
}
void* narrow_candidate_datatypes_c::visit(MOD_operator_c* symbol) {
    return narrow_binary_operator(widen_MOD_table, symbol);
}
void* narrow_candidate_datatypes_c::visit(GT_operator_c* symbol) {
    return narrow_binary_operator(widen_CMP_table, symbol);
}
...
/* visitors to CAL_operator_c, CALC_operator_c and CALCN_operator_c are called from visit(il_fb_call_c *)
 * {symbol->il_call_operator->accept(*this)} */
/* NOTE: The CAL, JMP and RET instructions simply set the desired datatype of the previous il instruction since they do
 * not change the value in the current/default IL variable */
...
void* narrow_candidate_datatypes_c::visit(CALC_operator_c* symbol) {
    return narrow_conditional_operator(symbol);
}
 ...
 * if (allow_enums) then consider that we are ectually processing an equ_expression or notequ_expression, where two
 * enums of the same data type may also be legally compared e.g.      symbol := l_expr == r_expr symbol := l_expr !=
 * r_expr In the above situation it is a legal operation when (l_expr.datatype == r_expr.datatype) &&
 * is_enumerated(r/l_expr.datatype) && is_bool(symbol.datatype)
 */
void* narrow_candidate_datatypes_c::narrow_binary_expression(const struct widen_entry widen_table[],
                                                             symbol_c* symbol,
                                                             symbol_c* l_expr,
                                                             symbol_c* r_expr,
                                                             bool* deprecated_operation,
        ...
        for (unsigned int j = 0; j < r_expr->candidate_datatypes.size(); j++) {
            ...
            if (is_widening_compatible(widen_table, l_type, r_type, symbol->datatype, deprecated_operation)) {
                ...
                count++;
                       ...
                       (get_datatype_info_c::is_BOOL_compatible(symbol->datatype) &&
                        get_datatype_info_c::is_enumerated(l_type) && (l_type == r_type))
                       /* handle the special case of comparison between REF_TO datatypes */
                       ||
                       (get_datatype_info_c::is_BOOL_compatible(symbol->datatype) &&
                        get_datatype_info_c::is_ref_to(l_type) && get_datatype_info_c::is_type_equal(l_type, r_type))) {
                if (NULL != deprecated_operation)
                    *deprecated_operation = false;
                l_expr->datatype = l_type;
                r_expr->datatype = r_type;
    ...
    if (get_datatype_info_c::is_type_valid(symbol->datatype) && (count <= 0))
        ERROR;
...
void* narrow_candidate_datatypes_c::narrow_equality_comparison(const struct widen_entry widen_table[],
                                                               symbol_c* symbol,
                                                               symbol_c* l_expr,
                                                               symbol_c* r_expr,
                                                               bool* deprecated_operation) {
...
void* narrow_candidate_datatypes_c::visit(or_expression_c* symbol) {
    return narrow_binary_expression(widen_OR_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* narrow_candidate_datatypes_c::visit(xor_expression_c* symbol) {
    return narrow_binary_expression(widen_XOR_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* narrow_candidate_datatypes_c::visit(and_expression_c* symbol) {
    return narrow_binary_expression(widen_AND_table, symbol, symbol->l_exp, symbol->r_exp);
}

void* narrow_candidate_datatypes_c::visit(equ_expression_c* symbol) {
    return narrow_equality_comparison(widen_CMP_table, symbol, symbol->l_exp, symbol->r_exp);
}
...
void* narrow_candidate_datatypes_c::visit(lt_expression_c* symbol) {
    return narrow_binary_expression(widen_CMP_table, symbol, symbol->l_exp, symbol->r_exp);
}
...
void* narrow_candidate_datatypes_c::visit(add_expression_c* symbol) {
    return narrow_binary_expression(widen_ADD_table,
                                    symbol,
                                    symbol->l_exp,
                                    symbol->r_exp,
                                    &symbol->deprecated_operation);
}
void* narrow_candidate_datatypes_c::visit(sub_expression_c* symbol) {
    return narrow_binary_expression(widen_SUB_table,
                                    symbol,
                                    symbol->l_exp,
                                    symbol->r_exp,
                                    &symbol->deprecated_operation);
...
void* narrow_candidate_datatypes_c::visit(mul_expression_c* symbol) {
    return narrow_binary_expression(widen_MUL_table,
                                    symbol,
                                    symbol->l_exp,
                                    symbol->r_exp,
                                    &symbol->deprecated_operation);
...
void* narrow_candidate_datatypes_c::visit(div_expression_c* symbol) {
    return narrow_binary_expression(widen_DIV_table,
                                    symbol,
                                    symbol->l_exp,
                                    symbol->r_exp,
                                    &symbol->deprecated_operation);
...
void* narrow_candidate_datatypes_c::visit(mod_expression_c* symbol) {
    return narrow_binary_expression(widen_MOD_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* narrow_candidate_datatypes_c::visit(power_expression_c* symbol) {
    return narrow_binary_expression(widen_EXPT_table, symbol, symbol->l_exp, symbol->r_exp);
}
 ...
 * pass data between the stage 3 and stage 4. */
/*    formal_param_list -> may be NULL ! */
/* nonformal_param_list -> may be NULL ! */
// SYM_REF3(function_invocation_c, function_name, formal_param_list, nonformal_param_list, symbol_c
// *called_function_declaration; int extensible_param_count; std::vector <symbol_c *> candidate_functions;)
...
void* narrow_candidate_datatypes_c::visit(for_statement_c* symbol) {
    ...
    for (unsigned int i = 0; i < symbol->beg_expression->candidate_datatypes.size(); i++) {
        if (get_datatype_info_c::is_type_equal(symbol->control_variable->datatype,
                                               symbol->beg_expression->candidate_datatypes[i]) &&
    ...
    for (unsigned int i = 0; i < symbol->end_expression->candidate_datatypes.size(); i++) {
        if (get_datatype_info_c::is_type_equal(symbol->control_variable->datatype,
                                               symbol->end_expression->candidate_datatypes[i]) &&
    ...
    if (NULL != symbol->by_expression) {
        for (unsigned int i = 0; i < symbol->by_expression->candidate_datatypes.size(); i++) {
            if (get_datatype_info_c::is_type_equal(symbol->control_variable->datatype,
                                                   symbol->by_expression->candidate_datatypes[i]) &&
