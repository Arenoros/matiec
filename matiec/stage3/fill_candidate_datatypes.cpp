...
/*
 *  Fill the candidate datatype list for all symbols that may legally 'have' a data type (e.g. variables, literals,
 ...
 *    BOOL, BYTE, WORD, DWORD, LWORD, USINT, SINT, UINT, INT, UDINT, DINT, ULINT, LINT (as well as the SAFE versions of
 * these data tyes too!)
 *
 * WARNING: This visitor class starts off by building a map of all enumeration constants that are defined in the source
 * code (i.e. a library_c symbol), and this map is later used to determine the datatpe of each use of an enumeration
 * constant. By implication, the fill_candidate_datatypes_c visitor class will only work corretly if it is asked to
 * visit a symbol of class library_c!!
 */

#include <main.h> /* required for UINT64_MAX, INT64_MAX, INT64_MIN, ... */
#include "fill_candidate_datatypes.h"
...
/* Add to the global_enumerated_value_symtable the global enum value constants, i.e. the enum constants used in the
 * enumerated datatypes that are defined inside a TYPE ... END_TYPE declaration.
 */
 ...
 *     END_VAR
 *     LocalEnumVar  := xxx1;   <-- We consider it OK!!!     xxx1 will reference the anonymous type used for
 * LocalEnumVar GlobalEnumVar := xxx1;   <-- We consider it an error. xxx1 will reference the anonymous type used for
 * LocalEnumVar GlobalEnumVar := GlobalEnumT#xxx1; END_FUNCTION_BLOCK
 */
...
public:
    ...
    void* visit(data_type_declaration_c* symbol) {
        return NULL;
    }  // do not visit the type declarations!!
    ...
    /* [enumerated_type_name '#'] identifier */
    void* visit(enumerated_value_c* symbol) {
        /* if the enumerated_value_c is not inside a enumerated_spec_init_c (e.g. used as the inital value of a
         * variable), we simply return */
        if (current_enumerated_type == NULL)
            return NULL;
        /* this is really an ERROR! The initial value may use the syntax NUM_TYPE#enum_value, but in that case we should
         * have return'd in the above statement !! */
        if (symbol->type != NULL)
            ERROR;
...
symbol_c* fill_candidate_datatypes_c::widening_conversion(symbol_c* left_type,
                                                          symbol_c* right_type,
...
bool fill_candidate_datatypes_c::add_datatype_to_candidate_list(symbol_c* symbol, symbol_c* datatype) {
     ...
     * NOTE: it reduces overall code size to do this test here, instead of doing every time before calling the
     * add_datatype_to_candidate_list() function.
     */
...
bool fill_candidate_datatypes_c::add_2datatypes_to_candidate_list(symbol_c* symbol,
                                                                  symbol_c* datatype1,
                                                                  symbol_c* datatype2) {
...
#define __REMOVE__(datatype)                                                                                           \
    ...
    { /* Remove unsigned data types */
        ...
        if (VALID_CVALUE(uint64, symbol))
            value = GET_CVALUE(uint64, symbol);
        if (IS_OVERFLOW(uint64, symbol))
            value = (uint64_t)UINT32_MAX + (uint64_t)1;

        if (value > 1) {
            __REMOVE__(bool_type_name);
        }
        if (value > UINT8_MAX) {
            __REMOVE__(usint_type_name);
            __REMOVE__(byte_type_name);
        }
        if (value > UINT16_MAX) {
            __REMOVE__(uint_type_name);
            __REMOVE__(word_type_name);
        }
        if (value > UINT32_MAX) {
            __REMOVE__(udint_type_name);
            __REMOVE__(dword_type_name);
        }
        if (IS_OVERFLOW(uint64, symbol)) {
            __REMOVE__(ulint_type_name);
            __REMOVE__(lword_type_name);
        }
    }

    { /* Remove signed data types */
        int64_t value = 0;
        if (VALID_CVALUE(int64v, symbol))
            value = GET_CVALUE(int64v, symbol);
        if (IS_OVERFLOW(int64v, symbol))
            value = (int64_t)INT32_MAX + (int64_t)1;

        if ((value < INT8_MIN) || (value > INT8_MAX)) {
            __REMOVE__(sint_type_name);
        }
        if ((value < INT16_MIN) || (value > INT16_MAX)) {
            __REMOVE__(int_type_name);
        }
        if ((value < INT32_MIN) || (value > INT32_MAX)) {
            __REMOVE__(dint_type_name);
        }
        if (IS_OVERFLOW(int64v, symbol)) {
            __REMOVE__(lint_type_name);
        }
    }
    ...
    { /* Remove floating point data types */
        ...
        if (VALID_CVALUE(real64, symbol))
            value = GET_CVALUE(real64, symbol);
        if (IS_OVERFLOW(real64, symbol))
            value = (real64_t)REAL32_MAX + (real64_t)1;
        if (value > REAL32_MAX) {
            __REMOVE__(real_type_name);
        }
        if (value < -REAL32_MAX) {
            __REMOVE__(real_type_name);
        }
        if (IS_OVERFLOW(real64, symbol)) {
            __REMOVE__(lreal_type_name);
        }
    }
#undef __REMOVE__
}
...
/* returns true if compatible function/FB invocation, otherwise returns false */
/* Assumes that the candidate_datatype lists of all the parameters being passed haved already been filled in */
bool fill_candidate_datatypes_c::match_formal_call(symbol_c* f_call,
                                                   symbol_c* f_decl,
                                                   symbol_c** first_param_datatype) {
...
void *fill_candidate_datatypes_c::handle_function_call(symbol_c *f_call, symbol_c *function_name, invocation_type_t
invocation_type, std::vector <symbol_c *> *candidate_datatypes, std::vector <symbol_c *> *candidate_functions) {
  */
void fill_candidate_datatypes_c::handle_function_call(symbol_c* fcall, generic_function_call_t fcall_data) {
    ...
    if (function_symtable.count(fcall_data.function_name) == 1) {
        ...
        if (add_datatype_to_candidate_list(fcall, returned_parameter_type))
            /* we only add it to the function declaration list if this entry was not already present in the candidate
             * datatype list! */
    ...
    for (; lower != upper; lower++) {
        ...
        if (NULL != fcall_data.nonformal_operand_list)
            compatible = match_nonformal_call(fcall, f_decl);
        if (NULL != fcall_data.formal_operand_list)
            compatible = match_formal_call(fcall, f_decl);
        if (compatible) {
            ...
            returned_parameter_type = base_type(f_decl->type_name);
            if (add_datatype_to_candidate_list(fcall, returned_parameter_type))
                /* we only add it to the function declaration list if this entry was not already present in the
                 * candidate datatype list! */
                fcall_data.candidate_functions.push_back(f_decl);
        }
    }
    if (debug)
        std::cout << "end_function() [" << fcall->candidate_datatypes.size() << "] result.\n";
    return;
}
 ...
 *        CU counter_var
 */
void* fill_candidate_datatypes_c::handle_implicit_il_fb_call(symbol_c* il_instruction,
    ...
    if (!get_datatype_info_c::is_function_block(fb_decl))
        fb_decl = NULL;

    /* Although a call to a non-declared FB is a semantic error, this is currently caught by stage 2! */
    /* However, when calling using the 'S' and 'R' operators, this error is not caught by stage 2, as these operators
     * have two possible semantics */
    // if (NULL == fb_type_id) ERROR;

    /* The narrow_candidate_datatypes_c does not rely on this called_fb_declaration pointer being == NULL to conclude
     * that we have a datatype incompatibility error, so we set it to fb_decl to allow the print_datatype_error_c to
     * print out more informative error messages!
     */
     ...
     * However, doing the above will often result in some very confusing error messages for the user, especially in the
     * case in which the FB call is wrong, so the resulting cadidate datatypes is an empty list. In this case, the user
     * would see many error messages related to the IL instructions that follow the FB call, even though those IL
     * instructions may be perfectly correct. For now, we will simply let the narrow_candidate_datatypes_c verify if the
     * datatypes are compatible (something that should be done here).
     */
    if (NULL != prev_il_instruction)
        il_instruction->candidate_datatypes = prev_il_instruction->candidate_datatypes;

    if (debug)
        std::cout << "handle_implicit_il_fb_call() [" << prev_il_instruction->candidate_datatypes.size() << "] ==> "
                  << il_instruction->candidate_datatypes.size() << " result.\n";
    return NULL;
}

/* handle the S and R IL operators... */
/* operator_str should be set to either "S" or "R" */
void* fill_candidate_datatypes_c::handle_S_and_R_operator(symbol_c* symbol,
    ...
    for (unsigned int i = 0; i < prev_il_instruction->candidate_datatypes.size(); i++) {
        for (unsigned int j = 0; j < il_operand->candidate_datatypes.size(); j++) {
             ...
             * IEC61131-3, Table 52, line 3 states that this operator should "Set operand to 1 if current result is
             * Boolean 1" which implies that the prev_instruction_type MUST also be BOOL compatible.
             */
            if (get_datatype_info_c::is_BOOL_compatible(prev_instruction_type) &&
                get_datatype_info_c::is_BOOL_compatible(operand_type))
    ...
    if (symbol->candidate_datatypes.size() == 0) {
        ...
        /* If it is also not a valid FB call, make sure the candidate_datatypes is empty (handle_implicit_il_fb_call may
         * leave it non-empty!!) */
        /* From here on out, all later code will consider the symbol->called_fb_declaration being NULL as an indication
         * that this operator must use the Set/Reset semantics, so we must also guarantee that the remainder of the
         * state of this symbol is compatible with that assumption!
         */
    ...
    if (debug)
        std::cout << operator_str << " [" << prev_il_instruction->candidate_datatypes.size() << ","
                  << il_operand->candidate_datatypes.size() << "] ==> " << symbol->candidate_datatypes.size()
                  << " result.\n";
    return NULL;
}

/* handle a binary IL operator, like ADD, SUB, etc... */
void* fill_candidate_datatypes_c::handle_binary_operator(const struct widen_entry widen_table[],
                                                         symbol_c* symbol,
                                                         symbol_c* l_expr,
                                                         symbol_c* r_expr) {
    if (NULL == l_expr)
        return NULL; /* if no prev_il_instruction */
    if (NULL == r_expr)
        return NULL; /* if no IL operand!! */
    ...
    for (unsigned int i = 0; i < l_expr->candidate_datatypes.size(); i++)
        for (unsigned int j = 0; j < r_expr->candidate_datatypes.size(); j++)
            ...
            add_datatype_to_candidate_list(symbol,
                                           widening_conversion(l_expr->candidate_datatypes[i],
                                                               r_expr->candidate_datatypes[j],
                                                               widen_table));
    remove_incompatible_datatypes(symbol);
    if (debug)
        std::cout << "[" << l_expr->candidate_datatypes.size() << "," << r_expr->candidate_datatypes.size() << "] ==> "
                  << symbol->candidate_datatypes.size() << " result.\n";
    return NULL;
}

/* handle a binary ST expression, like '+', '-', etc... */
void* fill_candidate_datatypes_c::handle_binary_expression(const struct widen_entry widen_table[],
                                                           symbol_c* symbol,
                                                           symbol_c* l_expr,
                                                           symbol_c* r_expr) {
    l_expr->accept(*this);
...
/* This function is special, as it will also allow enumeration data types to be compared, with the result being a BOOL
 * data type! It will also allow to REF_TO datatypes to be compared. These possibilities are not expressed in the
 * 'widening' tables, so we need to hard code it here
 */
void* fill_candidate_datatypes_c::handle_equality_comparison(const struct widen_entry widen_table[],
                                                             symbol_c* symbol,
                                                             symbol_c* l_expr,
                                                             symbol_c* r_expr) {
    ...
    for (unsigned int i = 0; i < l_expr->candidate_datatypes.size(); i++)
        for (unsigned int j = 0; j < r_expr->candidate_datatypes.size(); j++) {
            if ((get_datatype_info_c::is_enumerated(l_expr->candidate_datatypes[i]) &&
                 (l_expr->candidate_datatypes[i] == r_expr->candidate_datatypes[j])) ||
                (get_datatype_info_c::is_ref_to(l_expr->candidate_datatypes[i]) &&
                 get_datatype_info_c::is_type_equal(l_expr->candidate_datatypes[i], r_expr->candidate_datatypes[j])))
...
symbol_c* fill_candidate_datatypes_c::base_type(symbol_c* symbol) {
    /* NOTE: symbol == NULL is valid. It will occur when, for e.g., an undefined/undeclared symbolic_variable is used in
     * the code. */
    if (symbol == NULL)
        return NULL;
...
/* Before the existance of this derived_datatype_identifier_c class, all identifiers were stored in the generic
 * identifier_c class. Since not all identifiers are used to identify a datatype, the fill/narrow algorithms could not
 * do fill/narrow on all identifiers, and relied on the parent class to do the fill/narrow when apropriate (i.e. when
 * the identifier was known to reference a datatype for example, in a variable declaration -->  var1:
 ...
 * This means that we can now delete the code of all parents of derived_datatype_identifier_c that are still doing the
 * fill/narrow explicitly (assuming we do also implement the visitor for poutype_identifier_c). However, I will leave
 * this code cleanup for some later oportunity.
 */
void* fill_candidate_datatypes_c::visit(derived_datatype_identifier_c* symbol) {
    add_datatype_to_candidate_list(symbol,
                                   base_type(type_symtable[symbol->value]));  // will only add if datatype is not NULL!
    return NULL;
}

/* The datatype of a poutype_identifier_c is currently always being set by the parent object, so we leave this code
 * commented out for now. */
/*
void *fill_candidate_datatypes_c::visit(         poutype_identifier_c *symbol) {
  add_datatype_to_candidate_list(symbol, base_type( function_block_type_symtable[symbol->value])); // will only add if
datatype is not NULL! add_datatype_to_candidate_list(symbol, base_type(        program_type_symtable[symbol->value]));
// will only add if datatype is not NULL!
//add_datatype_to_candidate_list(symbol, base_type(            function_symtable[symbol->value])); // will only add if
...
void* fill_candidate_datatypes_c::handle_any_integer(symbol_c* symbol) {
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::bool_type_name,
                                     &get_datatype_info_c::safebool_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::byte_type_name,
                                     &get_datatype_info_c::safebyte_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::word_type_name,
                                     &get_datatype_info_c::safeword_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::dword_type_name,
                                     &get_datatype_info_c::safedword_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::lword_type_name,
                                     &get_datatype_info_c::safelword_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::sint_type_name,
                                     &get_datatype_info_c::safesint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::int_type_name,
                                     &get_datatype_info_c::safeint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::dint_type_name,
                                     &get_datatype_info_c::safedint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::lint_type_name,
                                     &get_datatype_info_c::safelint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::usint_type_name,
                                     &get_datatype_info_c::safeusint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::uint_type_name,
                                     &get_datatype_info_c::safeuint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::udint_type_name,
                                     &get_datatype_info_c::safeudint_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::ulint_type_name,
                                     &get_datatype_info_c::safeulint_type_name);
    remove_incompatible_datatypes(symbol);
    if (debug)
        std::cout << "ANY_INT [" << symbol->candidate_datatypes.size() << "]" << std::endl;
    return NULL;
}

void* fill_candidate_datatypes_c::handle_any_real(symbol_c* symbol) {
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::real_type_name,
                                     &get_datatype_info_c::safereal_type_name);
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::lreal_type_name,
                                     &get_datatype_info_c::safelreal_type_name);
    remove_incompatible_datatypes(symbol);
    if (debug)
        std::cout << "ANY_REAL [" << symbol->candidate_datatypes.size() << "]" << std::endl;
    return NULL;
}
...
void* fill_candidate_datatypes_c::handle_any_literal(symbol_c* symbol, symbol_c* symbol_value, symbol_c* symbol_type) {
    ...
    remove_incompatible_datatypes(symbol);
    if (debug)
        std::cout << "ANY_LITERAL [" << symbol->candidate_datatypes.size() << "]\n";
    return NULL;
}

void* fill_candidate_datatypes_c::visit(real_c* symbol) {
    return handle_any_real(symbol);
}
...
void* fill_candidate_datatypes_c::visit(neg_integer_c* symbol) {
    ...
    remove_incompatible_datatypes(symbol);
    if (debug)
        std::cout << "neg ANY_INT [" << symbol->candidate_datatypes.size() << "]" << std::endl;
    return NULL;
}

void* fill_candidate_datatypes_c::visit(integer_c* symbol) {
    return handle_any_integer(symbol);
}
...
void* fill_candidate_datatypes_c::visit(integer_literal_c* symbol) {
    return handle_any_literal(symbol, symbol->value, symbol->type);
}
...
void* fill_candidate_datatypes_c::visit(double_byte_character_string_c* symbol) {
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::wstring_type_name,
                                     &get_datatype_info_c::safewstring_type_name);
    return NULL;
}

void* fill_candidate_datatypes_c::visit(single_byte_character_string_c* symbol) {
    add_2datatypes_to_candidate_list(symbol,
                                     &get_datatype_info_c::string_type_name,
                                     &get_datatype_info_c::safestring_type_name);
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(duration_c* symbol) {
    ...
    if (debug)
        std::cout << "TIME_LITERAL [" << symbol->candidate_datatypes.size() << "]\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::fill_type_decl(symbol_c* symbol, symbol_c* type_name, symbol_c* spec_init) {
     ...
     *       Here we are essentially determining the base type of each defined data type. In many cases (especially
     * structs, enumerations, arrays, etc...), the datatype is its own base type. However, the derived datatype is
     * stored in multiple symbol_c classes (e.g. an enumeration uses enumerated_type_declaration_c,
     * enumerated_spec_init_c, enumerated_value_list_c, enumerated_value_c, ...). Several of these could be chosen to
     * work as the canonical base datatype symbol. Which symbol is used is really up to the search_base_type_c, and not
     * this fill_candidate_datatypes_c. Here we must right the code to handle whatever the search_base_type_c chooses to
     * use as the canonical symbol to represent the base datatype. Since the base datatype may be (and
     * sometimes/often/always(?) actually is) the top level symbol_c (an enumerated_type_declaration_c in the case of
     * the enumerations), it only makes sense to ask search_base_type_c for a basetype when we pass it the symbol in the
     * highest level of the type declaration (the enumerated_type_declaration_c in the case of the enumerations). For
     * this reason, we determine the basetype at the top level, and send that info down to the bottom level of the data
     * type declaration. In summary, a top->down algorithm!
     */
...
void* fill_candidate_datatypes_c::visit(simple_type_declaration_c* symbol) {
    return fill_type_decl(symbol, symbol->simple_type_name, symbol->simple_spec_init);
}
...
void* fill_candidate_datatypes_c::visit(subrange_type_declaration_c* symbol) {
    return fill_type_decl(symbol, symbol->subrange_type_name, symbol->subrange_spec_init);
}

/* subrange_specification ASSIGN signed_integer */
// SYM_REF2(subrange_spec_init_c, subrange_specification, signed_integer)
void* fill_candidate_datatypes_c::visit(subrange_spec_init_c* symbol) {
    return fill_spec_init(symbol, symbol->subrange_specification, symbol->signed_integer);
}
...
void* fill_candidate_datatypes_c::visit(subrange_c* symbol) {
    ...
    for (unsigned int u = 0; u < symbol->upper_limit->candidate_datatypes.size(); u++) {
        for (unsigned int l = 0; l < symbol->lower_limit->candidate_datatypes.size(); l++) {
            if (get_datatype_info_c::is_type_equal(symbol->upper_limit->candidate_datatypes[u],
                                                   symbol->lower_limit->candidate_datatypes[l]))
...
void* fill_candidate_datatypes_c::visit(enumerated_type_declaration_c* symbol) {
    return fill_type_decl(symbol, symbol->enumerated_type_name, symbol->enumerated_spec_init);
}
...
void* fill_candidate_datatypes_c::visit(enumerated_spec_init_c* symbol) {
    return fill_spec_init(symbol, symbol->enumerated_specification, symbol->enumerated_value);
}
...
void* fill_candidate_datatypes_c::visit(enumerated_value_list_c* symbol) {
    ...
    /* We already know the datatype of the enumerated_value(s) in the list, so we set them directly instead of
     * recursively calling the enumerated_value_c visit method! */
...
void* fill_candidate_datatypes_c::visit(enumerated_value_c* symbol) {
    ...
    if (NULL != symbol->type) {
        ...
        /* All local enum values are declared inside anonymous enumeration datatypes (i.e. inside a VAR ... END_VAR
         * declaration, with the enum type having no type name), so thay cannot possibly be referenced using a
         * datatype_t#enumvalue syntax. Because of this, we only look for the datatype identifier in the global enum
         * value symbol table!
         */
    ...
    if (debug)
        std::cout << "ENUMERATE [" << symbol->candidate_datatypes.size() << "]\n";
    return NULL;
}

/*  identifier ':' array_spec_init */
// SYM_REF2(array_type_declaration_c, identifier, array_spec_init)
void* fill_candidate_datatypes_c::visit(array_type_declaration_c* symbol) {
    return fill_type_decl(symbol, symbol->identifier, symbol->array_spec_init);
}
...
void* fill_candidate_datatypes_c::visit(structure_type_declaration_c* symbol) {
    return fill_type_decl(symbol, symbol->structure_type_name, symbol->structure_specification);
}

/* structure_type_name ASSIGN structure_initialization */
/* structure_initialization may be NULL ! */
// SYM_REF2(initialized_structure_c, structure_type_name, structure_initialization)
void* fill_candidate_datatypes_c::visit(initialized_structure_c* symbol) {
    return fill_spec_init(symbol, symbol->structure_type_name, symbol->structure_initialization);
}
...
void* fill_candidate_datatypes_c::visit(structure_element_initialization_list_c* symbol) {
    ...
    for (unsigned int i = 0; i < symbol->parent->candidate_datatypes.size();
         i++) {  // size() should always be 1 here -> a single structure or FB type!
        ...
        structure_element_declaration_list_c* struct_decl = dynamic_cast<structure_element_declaration_list_c*>(
            symbol->parent->candidate_datatypes[i]);
        // flag indicating all struct_elem->structure_element_name are structure elements found in the
        // symbol->parent->candidate_datatypes[i] datatype
        int flag_all_elem_ok = 1;  // assume all found
        for (int k = 0; k < symbol->size(); k++) {
            structure_element_initialization_c* struct_elem = dynamic_cast<structure_element_initialization_c*>(
                symbol->get_element(k));
            if (struct_elem == NULL)
                ERROR;
            ...
            if (struct_decl != NULL) {
                ...
                type = search_base_type_c::get_basetype_decl(
                    struct_decl->find_element(struct_elem->structure_element_name));
            } else {
            ...
            if (search_in_candidate_datatype_list(type, struct_elem->candidate_datatypes) < 0) {
                flag_all_elem_ok =
                    0;  // the necessary datatype for structure init element is not a candidate_datatype of that element
            }
        }
        if (flag_all_elem_ok) {
...
// SYM_REF4(string_type_declaration_c, string_type_name, elementary_string_type_name, string_type_declaration_size,
// string_type_declaration_init/* may be == NULL! */)

/*  function_block_type_name ASSIGN structure_initialization */
/* structure_initialization -> may be NULL ! */
// SYM_REF2(fb_spec_init_c, function_block_type_name, structure_initialization)
void* fill_candidate_datatypes_c::visit(fb_spec_init_c* symbol) {
    return fill_spec_init(symbol, symbol->function_block_type_name, symbol->structure_initialization);
}

/* REF_TO (non_generic_type_name | function_block_type_name) */
// SYM_REF1(ref_spec_c, type_name)
void* fill_candidate_datatypes_c::visit(ref_spec_c* symbol) {
    // when parsing datatype declarations, fill_candidate_datatypes_c follows a top->down algorithm (see the comment in
    // fill_type_decl() for an explanation)
...
void* fill_candidate_datatypes_c::visit(ref_spec_init_c* symbol) {
    return fill_spec_init(symbol, symbol->ref_spec, symbol->ref_initialization);
}

/* identifier ':' ref_spec_init */
// SYM_REF2(ref_type_decl_c, ref_type_name, ref_spec_init)
void* fill_candidate_datatypes_c::visit(ref_type_decl_c* symbol) {
    return fill_type_decl(symbol, symbol->ref_type_name, symbol->ref_spec_init);
}
...
void* fill_candidate_datatypes_c::visit(symbolic_variable_c* symbol) {
    ...
    add_datatype_to_candidate_list(symbol,
                                   search_var_instance_decl->get_basetype_decl(symbol)); /* will only add if non NULL */
    if (debug)
        std::cout << "VAR [" << symbol->candidate_datatypes.size() << "]\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(array_variable_c* symbol) {
    /* recursively call the subscripted_variable. We need to do this since the array variable may be stored inside a
     * structured variable (i.e. if it is an element inside a struct), in which case we want to recursively visit every
     * element of the struct, as it may contain more arrays whose subscripts must also be visited! e.g.
     * structvar.a1[v1+2].b1.c1[v2+3].d1 TYPE d_s: STRUCT d1: int; d2: int; d_a: ARRAY [1..3] OF d_s; c_s: STRUCT c1:
     * d_a; c2: d_a; b_s: STRUCT b1: c_s; b2: c_s; b_a: ARRAY [1..3] OF b_s; a_s: STRUCT a1: b_a; a2: b_a; END_TYPE VAR
     *          structvar: a_s;
     *        END_VAR
     */
    symbol->subscripted_variable->accept(*this);
    // the scope in which this variable was declared! It will be the same as the subscripted variable (a
    // symbolic_variable_ !)
    symbol->scope = symbol->subscripted_variable->scope;
    if (NULL == symbol->scope)
        ERROR;
    ...
    for (unsigned int i = 0; i < symbol->subscripted_variable->candidate_datatypes.size(); i++) {
        ...
        add_datatype_to_candidate_list(symbol,
                                       search_base_type_c::get_basetype_decl(
                                           get_datatype_info_c::get_array_storedtype_id(
                                               symbol->subscripted_variable
                                                   ->candidate_datatypes[i]))); /* will only add if non NULL */
    }
    ...
    if (debug)
        std::cout << "ARRAY_VAR [" << symbol->candidate_datatypes.size() << "]\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(structured_variable_c* symbol) {
     ...
     * The array subscripts may contain a complex expression (e.g. arrayvar[ varx + 33].elem1) whose datatype must be
     * correctly determined! The expression, may even contain a function call to an overloaded function! (e.g. arrayvar[
     * varx + TRUNC(realvar)].elem1)
     */
    ...
    if (symbol->record_variable->candidate_datatypes.size() == 1) {
        ...
        // We rely on the fact that if only one candidate datatype exists, then it will be the scope in which the
        // field_variable is declared!
        symbol->scope = symbol->record_variable->candidate_datatypes[0];  // the scope in which this variable was
                                                                          // declared! Will be used in stage4
        // Determine candidate datatypes...
        add_datatype_to_candidate_list(symbol,
                                       search_base_type_c::get_basetype_decl(
                                           get_datatype_info_c::get_struct_field_type_id(
                                               symbol->scope, symbol->field_selector))); /* will only add if non NULL */
    }
 ...
 *   (*    ^^^^  ^^^^  ^^^^                -> will NOT have the symbol->datatype set (for now, may change in the
 * future!) *)
...
void* fill_candidate_datatypes_c::visit(var1_init_decl_c* symbol) {
    return fill_var_declaration(symbol->var1_list, symbol->spec_init);
}
void* fill_candidate_datatypes_c::visit(array_var_init_decl_c* symbol) {
    return fill_var_declaration(symbol->var1_list, symbol->array_spec_init);
}
void* fill_candidate_datatypes_c::visit(structured_var_init_decl_c* symbol) {
    return fill_var_declaration(symbol->var1_list, symbol->initialized_structure);
}
void* fill_candidate_datatypes_c::visit(fb_name_decl_c* symbol) {
    return fill_var_declaration(symbol->fb_name_list, symbol->fb_spec_init);
}
void* fill_candidate_datatypes_c::visit(array_var_declaration_c* symbol) {
    return fill_var_declaration(symbol->var1_list, symbol->array_specification);
}
void* fill_candidate_datatypes_c::visit(structured_var_declaration_c* symbol) {
    return fill_var_declaration(symbol->var1_list, symbol->structure_type_name);
}
void* fill_candidate_datatypes_c::visit(external_declaration_c* symbol) {
    return fill_var_declaration(symbol->global_var_name, symbol->specification);
}
void* fill_candidate_datatypes_c::visit(global_var_decl_c* symbol) {
    return fill_var_declaration(symbol->global_var_spec, symbol->type_specification);
}
void* fill_candidate_datatypes_c::visit(incompl_located_var_decl_c* symbol) {
    return fill_var_declaration(symbol->variable_name, symbol->var_spec);
}
// void *fill_candidate_datatypes_c::visit(single_byte_string_var_declaration_c *symbol) {return
// handle_var_declaration(symbol->single_byte_string_spec);} void
// *fill_candidate_datatypes_c::visit(double_byte_string_var_declaration_c *symbol) {return
// handle_var_declaration(symbol->double_byte_string_spec);}

// NOTE: this method is not required since fill_candidate_datatypes_c inherits from iterator_visitor_c. TODO: delete
// this method!
...
void* fill_candidate_datatypes_c::visit(located_var_decl_c* symbol) {
    ...
    if (NULL != symbol->variable_name) {
        ...
        intersect_candidate_datatype_list(symbol->variable_name /*origin, dest.*/,
                                          symbol->located_var_spec_init /*with*/);
    }
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(function_declaration_c* symbol) {
    if (debug)
        printf("Filling candidate data types list of function %s\n",
...
void* fill_candidate_datatypes_c::visit(function_block_declaration_c* symbol) {
    if (debug)
        printf("Filling candidate data types list of FB %s\n", ((token_c*)(symbol->fblock_name))->value);
    local_enumerated_value_symtable.reset();
    current_scope = symbol;
...
void* fill_candidate_datatypes_c::visit(program_declaration_c* symbol) {
    if (debug)
        printf("Filling candidate data types list in program %s\n", ((token_c*)(symbol->program_type_name))->value);
    local_enumerated_value_symtable.reset();
    current_scope = symbol;
    symbol->var_declarations->accept(populate_enumvalue_symtable);
...
void* fill_candidate_datatypes_c::visit(configuration_declaration_c* symbol) {
    if (debug)
        printf("Filling candidate data types list in configuration %s\n",
    ...
    symbol->resource_declarations->accept(
        *this);  // points to a single_resource_declaration_c or a resource_declaration_list_c
...
void* fill_candidate_datatypes_c::visit(resource_declaration_c* symbol) {
    if (debug)
        printf("Filling candidate data types list in resource %s\n", ((token_c*)(symbol->resource_name))->value);
    //	local_enumerated_value_symtable.reset();  // TODO-> this must be replaced with
    //local_enumerated_value_symtable.push(), which is not yet implemented for the dsyntable_c!
    ...
    //	local_enumerated_value_symtable.reset();  // TODO-> this must be replaced with
    //local_enumerated_value_symtable.pop(), which is not yet implemented for the dsyntable_c!
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(il_instruction_c* symbol) {
    if (NULL == symbol->il_instruction) {
        ...
        if (symbol->prev_il_instruction.size() == 0)
            ...
            prev_il_instruction = &fake_prev_il_instruction;
...
/* NOTE: The parameters 'called_function_declaration' and 'extensible_param_count' are used to pass data between the
 * stage 3 and stage 4. */
// SYM_REF2(il_function_call_c, function_name, il_operand_list, symbol_c *called_function_declaration; int
// extensible_param_count;)
void* fill_candidate_datatypes_c::visit(il_function_call_c* symbol) {
    /* The first parameter of a non formal function call in IL will be the 'current value' (i.e. the
     * prev_il_instruction) In order to be able to handle this without coding special cases, we will simply prepend that
     * symbol to the il_operand_list, and remove it after calling handle_function_call().
     *
    ...
    if (NULL == symbol->il_operand_list)
        symbol->il_operand_list = new il_operand_list_c;
    if (NULL == symbol->il_operand_list)
        ERROR;
    ...
    if (((list_c*)symbol->il_operand_list)->size() == 0) {
        /* if the list becomes empty, then that means that it did not exist before we made these changes, so we delete
         * it! */
    ...
    if (debug)
        std::cout << "il_function_call_c [" << symbol->candidate_datatypes.size() << "] result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(il_expression_c* symbol) {
    ...
    /* Stage2 will insert an artificial (and equivalent) LD <il_operand> to the simple_instr_list if necessary. We can
     * therefore ignore the 'il_operand' entry! */
    // if (NULL != symbol->il_operand)
    //   symbol->il_operand->accept(*this);
    ...
    /* Since stage2 will insert an artificial (and equivalent) LD <il_operand> to the simple_instr_list when an
     * 'il_operand' exists, we know that if (symbol->il_operand != NULL), then the first IL instruction in the
     * simple_instr_list will be the equivalent and artificial 'LD <il_operand>' IL instruction. Just to be cosistent,
     * we will copy the datatype info back into the il_operand, even though this should not be necessary!
     */
    if ((NULL != symbol->il_operand) &&
        ((NULL == symbol->simple_instr_list) || (0 == ((list_c*)symbol->simple_instr_list)->size())))
        ERROR;  // stage2 is not behaving as we expect it to!
    if (NULL != symbol->il_operand)
        symbol->il_operand
            ->candidate_datatypes = ((list_c*)symbol->simple_instr_list)->get_element(0)->candidate_datatypes;
...
void* fill_candidate_datatypes_c::visit(il_fb_call_c* symbol) {
    ...
    if (debug)
        std::cout << "FB [] ==> " << symbol->candidate_datatypes.size() << " result.\n";
    return NULL;
}

/* | function_name '(' eol_list [il_param_list] ')' */
/* NOTE: The parameter 'called_function_declaration' is used to pass data between the stage 3 and stage 4. */
// SYM_REF2(il_formal_funct_call_c, function_name, il_param_list, symbol_c *called_function_declaration; int
// extensible_param_count;)
void* fill_candidate_datatypes_c::visit(il_formal_funct_call_c* symbol) {
    ...
    if (debug)
        std::cout << "il_formal_funct_call_c [" << symbol->candidate_datatypes.size() << "] result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(simple_instr_list_c* symbol) {
    ...
    symbol->candidate_datatypes = symbol->get_element(symbol->size() - 1)->candidate_datatypes;
...
void* fill_candidate_datatypes_c::visit(il_simple_instruction_c* symbol) {
    if (symbol->prev_il_instruction.size() > 1)
        ERROR; /* There should be no labeled insructions inside an IL expression! */
    if (symbol->prev_il_instruction.size() == 0)
        prev_il_instruction = NULL;
    else
        prev_il_instruction = symbol->prev_il_instruction[0];
...
void* fill_candidate_datatypes_c::visit(LD_operator_c* symbol) {
    ...
    if (debug)
        std::cout << "LD [" << il_operand->candidate_datatypes.size() << "] ==> " << symbol->candidate_datatypes.size()
                  << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(LDN_operator_c* symbol) {
    ...
    if (debug)
        std::cout << "LDN [" << il_operand->candidate_datatypes.size() << "] ==> " << symbol->candidate_datatypes.size()
                  << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(ST_operator_c* symbol) {
    ...
    if (debug)
        std::cout << "ST [" << prev_il_instruction->candidate_datatypes.size() << ","
                  << il_operand->candidate_datatypes.size() << "] ==> " << symbol->candidate_datatypes.size()
                  << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(STN_operator_c* symbol) {
    ...
    for (unsigned int i = 0; i < prev_il_instruction->candidate_datatypes.size(); i++) {
        for (unsigned int j = 0; j < il_operand->candidate_datatypes.size(); j++) {
            ...
            operand_type = il_operand->candidate_datatypes[j];
            if (get_datatype_info_c::is_type_equal(prev_instruction_type, operand_type) &&
                get_datatype_info_c::is_ANY_BIT_compatible(operand_type))
                add_datatype_to_candidate_list(symbol, prev_instruction_type);
        }
    }
    if (debug)
        std::cout << "STN [" << prev_il_instruction->candidate_datatypes.size() << ","
                  << il_operand->candidate_datatypes.size() << "] ==> " << symbol->candidate_datatypes.size()
                  << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(NOT_operator_c* symbol) {
    ...
    if (debug)
        std::cout << "NOT_operator [" << prev_il_instruction->candidate_datatypes.size() << "] ==> "
                  << symbol->candidate_datatypes.size() << " result.\n";
    return NULL;
}

void* fill_candidate_datatypes_c::visit(S_operator_c* symbol) {
    return handle_S_and_R_operator(symbol, "S", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(R_operator_c* symbol) {
    return handle_S_and_R_operator(symbol, "R", symbol->called_fb_declaration);
}

void* fill_candidate_datatypes_c::visit(S1_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "S1", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(R1_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "R1", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(CLK_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "CLK", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(CU_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "CU", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(CD_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "CD", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(PV_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "PV", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(IN_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "IN", symbol->called_fb_declaration);
}
void* fill_candidate_datatypes_c::visit(PT_operator_c* symbol) {
    return handle_implicit_il_fb_call(symbol, "PT", symbol->called_fb_declaration);
}

void* fill_candidate_datatypes_c::visit(AND_operator_c* symbol) {
    return handle_binary_operator(widen_AND_table, symbol, prev_il_instruction, il_operand);
}
void* fill_candidate_datatypes_c::visit(OR_operator_c* symbol) {
    return handle_binary_operator(widen_OR_table, symbol, prev_il_instruction, il_operand);
}
void* fill_candidate_datatypes_c::visit(XOR_operator_c* symbol) {
    return handle_binary_operator(widen_XOR_table, symbol, prev_il_instruction, il_operand);
}
...
void* fill_candidate_datatypes_c::visit(ADD_operator_c* symbol) {
    return handle_binary_operator(widen_ADD_table, symbol, prev_il_instruction, il_operand);
}
void* fill_candidate_datatypes_c::visit(SUB_operator_c* symbol) {
    return handle_binary_operator(widen_SUB_table, symbol, prev_il_instruction, il_operand);
}
void* fill_candidate_datatypes_c::visit(MUL_operator_c* symbol) {
    return handle_binary_operator(widen_MUL_table, symbol, prev_il_instruction, il_operand);
}
void* fill_candidate_datatypes_c::visit(DIV_operator_c* symbol) {
    return handle_binary_operator(widen_DIV_table, symbol, prev_il_instruction, il_operand);
}
void* fill_candidate_datatypes_c::visit(MOD_operator_c* symbol) {
    return handle_binary_operator(widen_MOD_table, symbol, prev_il_instruction, il_operand);
}

void* fill_candidate_datatypes_c::visit(GT_operator_c* symbol) {
    return handle_binary_operator(widen_CMP_table, symbol, prev_il_instruction, il_operand);
}
...
void* fill_candidate_datatypes_c::visit(CAL_operator_c* symbol) {
    if (NULL != prev_il_instruction)
        symbol->candidate_datatypes = prev_il_instruction->candidate_datatypes;
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(CALC_operator_c* symbol) {
    return handle_conditional_il_flow_control_operator(symbol);
}
 ...
 * standard. Returns address of the varible! */
 ...
 * Returns address of the varible! */
...
void* fill_candidate_datatypes_c::visit(ref_expression_c* symbol) {
    ...
    /* Currently the IEC 61131-3 symtax requires that the REF() operator have as a parameter a lvalue (a variable), that
     * will have at most one candidate_datatype. This means that we do not really need the for() loop here, but we use
     * it anyway as it is the correct way of implementing the fill/narrow algorithm!
     */
    for (unsigned int i = 0; i < symbol->exp->candidate_datatypes.size(); i++) {
...
void* fill_candidate_datatypes_c::visit(or_expression_c* symbol) {
    return handle_binary_expression(widen_OR_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(xor_expression_c* symbol) {
    return handle_binary_expression(widen_XOR_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(and_expression_c* symbol) {
    return handle_binary_expression(widen_AND_table, symbol, symbol->l_exp, symbol->r_exp);
}

void* fill_candidate_datatypes_c::visit(equ_expression_c* symbol) {
    return handle_equality_comparison(widen_CMP_table, symbol, symbol->l_exp, symbol->r_exp);
}
...
void* fill_candidate_datatypes_c::visit(lt_expression_c* symbol) {
    return handle_binary_expression(widen_CMP_table, symbol, symbol->l_exp, symbol->r_exp);
}
...
void* fill_candidate_datatypes_c::visit(add_expression_c* symbol) {
    return handle_binary_expression(widen_ADD_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(sub_expression_c* symbol) {
    return handle_binary_expression(widen_SUB_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(mul_expression_c* symbol) {
    return handle_binary_expression(widen_MUL_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(div_expression_c* symbol) {
    return handle_binary_expression(widen_DIV_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(mod_expression_c* symbol) {
    return handle_binary_expression(widen_MOD_table, symbol, symbol->l_exp, symbol->r_exp);
}
void* fill_candidate_datatypes_c::visit(power_expression_c* symbol) {
    return handle_binary_expression(widen_EXPT_table, symbol, symbol->l_exp, symbol->r_exp);
}
...
void* fill_candidate_datatypes_c::visit(neg_expression_c* symbol) {
    ...
    if (debug)
        std::cout << "neg [" << symbol->exp->candidate_datatypes.size() << "] ==> "
                  << symbol->candidate_datatypes.size() << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(not_expression_c* symbol) {
    ...
    if (debug)
        std::cout << "not [" << symbol->exp->candidate_datatypes.size() << "] ==> "
                  << symbol->candidate_datatypes.size() << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(function_invocation_c* symbol) {
    ...
    // else ERROR;  NOTE-> We support the non-standard feature of POUS with no in, out and inout parameters, so this is
    // no longer an internal error!

    generic_function_call_t fcall_param = {/*function_name:*/ symbol->function_name,
    ...
    if (debug)
        std::cout << "function_invocation_c [" << symbol->candidate_datatypes.size() << "] result.\n";
    return NULL;
}
    ...
    for (unsigned int i = 0; i < symbol->l_exp->candidate_datatypes.size(); i++) {
        for (unsigned int j = 0; j < symbol->r_exp->candidate_datatypes.size(); j++) {
            ...
            if (get_datatype_info_c::is_type_equal(left_type, right_type))
                add_datatype_to_candidate_list(symbol, left_type);  // NOTE: Must use left_type, as the right_type may
                                                                    // be the 'NULL' reference! (see comment in
                                                                    // visit(ref_value_null_literal_c)) */
        }
    }
    if (debug)
        std::cout << ":= [" << symbol->l_exp->candidate_datatypes.size() << ","
                  << symbol->r_exp->candidate_datatypes.size() << "] ==> " << symbol->candidate_datatypes.size()
                  << " result.\n";
    return NULL;
}
...
void* fill_candidate_datatypes_c::visit(fb_invocation_c* symbol) {
    ...
    if (!get_datatype_info_c::is_function_block(fb_decl))
        fb_decl = NULL;
    if (NULL == fb_decl)
        ERROR; /* Although a call to a non-declared FB is a semantic error, this is currently caught by stage 2! */
