...
static bool less_than(symbol_c* s1, symbol_c* s2) {
    if ((VALID_CVALUE(int64v, s1)) && (VALID_CVALUE(int64v, s2)) && (GET_CVALUE(int64v, s1) < GET_CVALUE(int64v, s2)))
        return true;

    if ((VALID_CVALUE(uint64, s1)) && (VALID_CVALUE(uint64, s2)) && (GET_CVALUE(uint64, s1) < GET_CVALUE(uint64, s2)))
        return true;

    if ((VALID_CVALUE(int64v, s1)) && (VALID_CVALUE(uint64, s2)) && (GET_CVALUE(int64v, s1) < 0))
        return true;
...
void case_elements_check_c::check_subr_subr(symbol_c* s1, symbol_c* s2) {
    ...
    if ((VALID_CVALUE(int64v, l1) || (VALID_CVALUE(uint64, l1))) &&
        (VALID_CVALUE(int64v, l2) || (VALID_CVALUE(uint64, l2))) &&
        (VALID_CVALUE(int64v, u1) || (VALID_CVALUE(uint64, u1))) &&
        (VALID_CVALUE(int64v, u2) || (VALID_CVALUE(uint64, u2))))
...
void case_elements_check_c::check_subr_symb(symbol_c* s1, symbol_c* s2) {
    ...
    symbol_c* symb = NULL;
    if ((subr = dynamic_cast<subrange_c*>(s1)) != NULL) {
        symb = s2;
    }
    if ((subr = dynamic_cast<subrange_c*>(s2)) != NULL) {
        symb = s1;
    }

    if ((NULL == subr) || (NULL == symb))
        return;
    symbol_c* lowl = subr->lower_limit;
    symbol_c* uppl = subr->upper_limit;

    if ((VALID_CVALUE(int64v, symb)) && (VALID_CVALUE(int64v, lowl)) && (VALID_CVALUE(int64v, uppl)) &&
        (GET_CVALUE(int64v, symb) >= GET_CVALUE(int64v, lowl)) &&
        (GET_CVALUE(int64v, symb) <= GET_CVALUE(int64v, uppl))) {
        STAGE3_WARNING(s1, s2, "Element in CASE option falls within range of another element.");
        return;
    }

    if ((VALID_CVALUE(uint64, symb)) && (VALID_CVALUE(int64v, lowl)) && (VALID_CVALUE(uint64, uppl)) &&
        (GET_CVALUE(int64v, lowl) < 0) && (GET_CVALUE(uint64, symb) <= GET_CVALUE(uint64, uppl))) {
        STAGE3_WARNING(s1, s2, "Element in CASE option falls within range of another element.");
        return;
    }

    if ((VALID_CVALUE(uint64, symb)) && (VALID_CVALUE(uint64, lowl)) && (VALID_CVALUE(uint64, uppl)) &&
...
void case_elements_check_c::check_symb_symb(symbol_c* s1, symbol_c* s2) {
    if ((dynamic_cast<subrange_c*>(s1) != NULL) || (dynamic_cast<subrange_c*>(s2) != NULL))
        return;  // only run this test if neither s1 nor s2 are subranges!

    if ((s1->const_value.is_const() && s2->const_value.is_const() &&
         (s1->const_value == s2->const_value))  // if const, then compare const values (using overloaded '==' operator!)
        || (compare_identifiers(s1, s2) == 0))  // if token_c, compare tokens! (compare_identifiers() returns 0 when
                                                // equal tokens!, -1 when either is not token_c)
...
void* case_elements_check_c::visit(case_statement_c* symbol) {
    std::vector<symbol_c*> case_elements_list_local =
        case_elements_list;  // Required when source code contains CASE inside another CASE !
