...
private:
    /* Pointer to the previous IL instruction, which contains the current cvalue of the data stored in the IL stack,
     * i.e. the default variable, a.k.a. accumulator */
...
private:
    ...
    std::deque<function_block_declaration_c*>
        fbs_currently_being_visited;  // We use a deque instead of stack, so we can search in the stack using direct
    ...
    // Flag to indicate whether the variables in the variable declaration list will always have a fixed value when the
    // POU is executed! VAR CONSTANT ... END_VAR will always be true VAR          ... END_VAR will always be true for
    // functions (who initialise local variables every time they are called), but false for FBs and PROGRAMS
