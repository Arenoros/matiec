...
#include "main.h"  // required for uint8_t, real_64_t, ..., and the macros NAN, INFINITY, INT8_MAX, REAL32_MAX, ... */
...
#define STAGE3_ERROR(error_level, symbol1, symbol2, ...)                                                               \
    {                                                                                                                  \
        if (current_display_error_level >= error_level) {                                                              \
            fprintf(stderr,                                                                                            \
                    "%s:%d-%d..%d-%d: error: ",                                                                        \
                    FIRST_(symbol1, symbol2)->first_file,                                                              \
                    FIRST_(symbol1, symbol2)->first_line,                                                              \
                    FIRST_(symbol1, symbol2)->first_column,                                                            \
                    LAST_(symbol1, symbol2)->last_line,                                                                \
                    LAST_(symbol1, symbol2)->last_column);                                                             \
...
#define ISEQUAL_CVALUE(dtype, symbol1, symbol2)                                                                        \
    (VALID_CVALUE(dtype, symbol1) && VALID_CVALUE(dtype, symbol2) &&                                                   \
     (GET_CVALUE(dtype, symbol1) == GET_CVALUE(dtype, symbol2)))
...
#define DO_BINARY_OPER(oper_type, operation, res_type, operand1, operand2)                                             \
    {                                                                                                                  \
        if (VALID_CVALUE(oper_type, operand1) && VALID_CVALUE(oper_type, operand2)) {                                  \
            ...
            SET_OVFLOW(res_type, symbol);                                                                              \
        } /* does it really make sense to set OVFLOW when restype is boolean??  */                                     \
 ...
 */
static int64_t matiec_strtoint64(long int* dummy, const char* nptr, char** endptr, int base) {
    return strtol(nptr, endptr, base);
}
static int64_t matiec_strtoint64(long long int* dummy, const char* nptr, char** endptr, int base) {
    return strtoll(nptr, endptr, base);
}

static uint64_t matiec_strtouint64(unsigned long int* dummy, const char* nptr, char** endptr, int base) {
    return strtoul(nptr, endptr, base);
}
static uint64_t matiec_strtouint64(unsigned long long int* dummy, const char* nptr, char** endptr, int base) {
...
int64_t extract_int64_value(symbol_c* sym, bool* overflow) {
    ...
    if ((integer = dynamic_cast<integer_c*>(sym)) != NULL) {
        value = integer->value + 0;
        base = 10;
    } else if ((hex_integer = dynamic_cast<hex_integer_c*>(sym)) != NULL) {
        value = hex_integer->value + 3;
        base = 16;
    } else if ((octal_integer = dynamic_cast<octal_integer_c*>(sym)) != NULL) {
        value = octal_integer->value + 2;
        base = 8;
    } else if ((binary_integer = dynamic_cast<binary_integer_c*>(sym)) != NULL) {
        value = binary_integer->value + 2;
        base = 2;
    } else
        ERROR;
 ...
 */
double extract_real_value(symbol_c* sym, bool* overflow) {
    std::string str = "";
    real_c* real_sym;
...
static void CHECK_OVERFLOW_int64_SUM(symbol_c* res, symbol_c* a_ptr, symbol_c* b_ptr) {
    if (!VALID_CVALUE(int64v, res))
        return;
    int64_t a = GET_CVALUE(int64v, a_ptr);
    int64_t b = GET_CVALUE(int64v, b_ptr);
    /* The following test is valid no matter what representation is being used (e.g. two's complement, etc...) */
    if (((b > 0) && (a > (INT64_MAX - b))) || ((b < 0) && (a < (INT64_MIN - b))))
        SET_OVFLOW(int64v, res);
}
...
static void CHECK_OVERFLOW_int64_MUL(symbol_c* res, symbol_c* a_ptr, symbol_c* b_ptr) {
    ...
    if (((a > 0) && (b > 0) && (a > (INT64_MAX / b))) || ((a > 0) && !(b > 0) && (b < (INT64_MIN / a))) ||
        (!(a > 0) && (b > 0) && (a < (INT64_MIN / b))) || (!(a > 0) && !(b > 0) && (a != 0) && (b < (INT64_MAX / a))))
        SET_OVFLOW(int64v, res);
}
...
static void CHECK_OVERFLOW_int64_NEG(symbol_c* res, symbol_c* a) {
    if (!VALID_CVALUE(int64v, res))
        return;
    if (GET_CVALUE(int64v, a) == INT64_MIN)
        SET_OVFLOW(int64v, res);
}
 ...
 *   The result of comparig the bool, int64v, and uint64 are overwritten by the comparison of the real64 type!
 */
...
static void* handle_neg(symbol_c* symbol, symbol_c* oper) {
    if (NULL == oper)
        return NULL;
    /* NOTE: The oper may never be an integer/real literal, '-1' and '-2.2' are stored as an neg_integer_c/neg_real_c
     * instead. Because of this, we MUST NOT handle the INT_MIN special situation that is handled in neg_integer_c
     * visitor!
     *
    ...
    CHECK_OVERFLOW_uint64_NEG(symbol, oper); /* handle the uint_v := -0 situation! */
    DO_UNARY_OPER(int64v, -, oper);
    CHECK_OVERFLOW_int64_NEG(symbol, oper);
    DO_UNARY_OPER(real64, -, oper);
...
static void* handle_add(symbol_c* symbol, symbol_c* oper1, symbol_c* oper2) {
    if ((NULL == oper1) || (NULL == oper2))
        return NULL;
    DO_BINARY_OPER(uint64, +, uint64, oper1, oper2);
    CHECK_OVERFLOW_uint64_SUM(symbol, oper1, oper2);
    DO_BINARY_OPER(int64v, +, int64v, oper1, oper2);
    CHECK_OVERFLOW_int64_SUM(symbol, oper1, oper2);
    DO_BINARY_OPER(real64, +, real64, oper1, oper2);
    CHECK_OVERFLOW_real64(symbol);
    return NULL;
}
...
static void* handle_sub(symbol_c* symbol, symbol_c* oper1, symbol_c* oper2) {
    if ((NULL == oper1) || (NULL == oper2))
        return NULL;
    DO_BINARY_OPER(uint64, -, uint64, oper1, oper2);
    CHECK_OVERFLOW_uint64_SUB(symbol, oper1, oper2);
    DO_BINARY_OPER(int64v, -, int64v, oper1, oper2);
    CHECK_OVERFLOW_int64_SUB(symbol, oper1, oper2);
    DO_BINARY_OPER(real64, -, real64, oper1, oper2);
    CHECK_OVERFLOW_real64(symbol);
    return NULL;
}
...
static void* handle_mul(symbol_c* symbol, symbol_c* oper1, symbol_c* oper2) {
    if ((NULL == oper1) || (NULL == oper2))
        return NULL;
    DO_BINARY_OPER(uint64, *, uint64, oper1, oper2);
    CHECK_OVERFLOW_uint64_MUL(symbol, oper1, oper2);
    DO_BINARY_OPER(int64v, *, int64v, oper1, oper2);
    CHECK_OVERFLOW_int64_MUL(symbol, oper1, oper2);
    DO_BINARY_OPER(real64, *, real64, oper1, oper2);
    CHECK_OVERFLOW_real64(symbol);
    return NULL;
}
...
static void* handle_div(symbol_c* symbol, symbol_c* oper1, symbol_c* oper2) {
    if ((NULL == oper1) || (NULL == oper2))
        return NULL;
    if (ISZERO_CVALUE(uint64, oper2)) {
        SET_OVFLOW(uint64, symbol);
    } else {
        DO_BINARY_OPER(uint64, /, uint64, oper1, oper2);
        CHECK_OVERFLOW_uint64_DIV(symbol, oper1, oper2);
    };
    if (ISZERO_CVALUE(int64v, oper2)) {
        SET_OVFLOW(int64v, symbol);
    } else {
        DO_BINARY_OPER(int64v, /, int64v, oper1, oper2);
        CHECK_OVERFLOW_int64_DIV(symbol, oper1, oper2);
    };
    if (ISZERO_CVALUE(real64, oper2)) {
...
static void* handle_mod(symbol_c* symbol, symbol_c* oper1, symbol_c* oper2) {
     ...
     */
    if (ISZERO_CVALUE(uint64, oper2)) {
        SET_CVALUE(uint64, symbol, 0);
    } else {
        DO_BINARY_OPER(uint64, %, uint64, oper1, oper2);
        CHECK_OVERFLOW_uint64_MOD(symbol, oper1, oper2);
    };
    if (ISZERO_CVALUE(int64v, oper2)) {
        SET_CVALUE(int64v, symbol, 0);
    } else {
        DO_BINARY_OPER(int64v, %, int64v, oper1, oper2);
        CHECK_OVERFLOW_int64_MOD(symbol, oper1, oper2);
    };
    return NULL;
}

static void* handle_pow(symbol_c* symbol, symbol_c* oper1, symbol_c* oper2) {
    /* NOTE: If the const_value in symbol->r_exp is within the limits of both int64 and uint64, then we do both
     * operations. That is OK, as the result should be identicial (we do create an unnecessary CVALUE variable, but who
     * cares?). If only one is valid, then that is the oper we will do!
     */
    if (VALID_CVALUE(real64, oper1) && VALID_CVALUE(int64v, oper2))
        SET_CVALUE(real64, symbol, pow(GET_CVALUE(real64, oper1), GET_CVALUE(int64v, oper2)));
...
/* If the cvalues of all the prev_il_intructions have the same VALID value, then set the local cvalue to that value,
 * otherwise, set it to NONCONST! */
...
static void intersect_prev_cvalues(il_instruction_c* symbol) {
    ...
    intersect_prev_CVALUE_(int64v, symbol);
...
constant_folding_c::constant_folding_c(symbol_c* symbol) {
    ...
    if (!(std::numeric_limits<real64_t>::is_iec559))
        STAGE3_WARNING(
            &null_symbol,
            &null_symbol,
            "The platform running the compiler does not implement IEC 60559 floating point numbers. "
            "Any error and/or warning messages related to overflow/underflow of the result of operations on REAL/LREAL "
            "literals "
...
void* constant_folding_c::visit(integer_c* symbol) {
    bool overflow;
    SET_CVALUE(int64v, symbol, extract_int64_value(symbol, &overflow));
    if (overflow)
        SET_OVFLOW(int64v, symbol);
...
void* constant_folding_c::visit(neg_real_c* symbol) {
    ...
    CHECK_OVERFLOW_real64(symbol);
    if (IS_OVFLOW(real64, symbol->exp))
        SET_OVFLOW(real64, symbol);
    return NULL;
}
...
void* constant_folding_c::visit(neg_integer_c* symbol) {
    ...
    CHECK_OVERFLOW_uint64_NEG(symbol, symbol->exp); /* handle the uintv := -0 situation */
    if (IS_OVFLOW(uint64, symbol->exp))
        SET_OVFLOW(uint64, symbol);
    DO_UNARY_OPER(int64v, -, symbol->exp);
    CHECK_OVERFLOW_int64_NEG(symbol, symbol->exp);
    if (IS_OVFLOW(int64v, symbol->exp))
        SET_OVFLOW(int64v, symbol);
    /* NOTE 1: INT64_MIN = -(INT64_MAX + 1)   ---> assuming two's complement representation!!!
     * NOTE 2: if the user happens to want INT_MIN, that value will first be parsed as a positive integer, before being
     * negated here. However, the positive value cannot be stored inside an int64! So, in this case, we will get the
     * value from the uint64 cvalue.
     *
     * This same situation is usually considered an overflow (check handle_neg() function). However, here we have a
     * special situation. If we do not allow this, then the user would never the able to use the following code: VAR v :
     * LINT; END_VAR v := -9223372036854775809 ; (* - |INT64_MIN| == INT64_MIN *)
     */
    ...
    if (VALID_CVALUE(uint64, symbol->exp) && (GET_CVALUE(uint64, symbol->exp) == (uint64_t)INT64_MAX + 1)) {
        SET_CVALUE(int64v, symbol, INT64_MIN);
    }
    return NULL;
}
...
void* constant_folding_c::visit(integer_literal_c* symbol) {
    ...
    DO_UNARY_OPER(int64v, /* none */, symbol->value);
...
// extensible_param_count;) void *constant_folding_c::visit(il_function_call_c *symbol) {}
...
void* constant_folding_c::visit(il_expression_c* symbol) {
     ...
     * we will copy the constant info back into the il_operand, even though this should not be necessary!
     */
    if ((NULL != symbol->il_operand) &&
...
void* constant_folding_c::visit(il_fb_call_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
...
// extensible_param_count;) void *constant_folding_c::visit(il_formal_funct_call_c *symbol) {return NULL;}
...
void* constant_folding_c::visit(simple_instr_list_c* symbol) {
    ...
    /* This object has (inherits) the same cvalues as the il_jump_operator */
    symbol->const_value = symbol->get_element(symbol->size() - 1)->const_value;
    return NULL;
}
...
/* B 2.2 Operators */
/*******************/
void* constant_folding_c::visit(LD_operator_c* symbol) {
    return handle_move(symbol, il_operand);
}
void* constant_folding_c::visit(LDN_operator_c* symbol) {
    return handle_not(symbol, il_operand);
}
...
void* constant_folding_c::visit(ST_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(STN_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
 ...
 */
void* constant_folding_c::visit(NOT_operator_c* symbol) {
    return handle_not(symbol, prev_il_instruction);
}
 ...
 */
void* constant_folding_c::visit(S_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(R_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}

/* FB calls leave the value in the accumulator unchanged */
void* constant_folding_c::visit(S1_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(R1_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(CLK_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(CU_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(CD_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(PV_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(IN_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(PT_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}

void* constant_folding_c::visit(AND_operator_c* symbol) {
    return handle_and(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(OR_operator_c* symbol) {
    return handle_or(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(XOR_operator_c* symbol) {
    return handle_xor(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(ANDN_operator_c* symbol) {
    handle_and(symbol, prev_il_instruction, il_operand);
    return handle_not(symbol, symbol);
}
void* constant_folding_c::visit(ORN_operator_c* symbol) {
    handle_or(symbol, prev_il_instruction, il_operand);
    return handle_not(symbol, symbol);
}
void* constant_folding_c::visit(XORN_operator_c* symbol) {
    handle_xor(symbol, prev_il_instruction, il_operand);
    return handle_not(symbol, symbol);
}

void* constant_folding_c::visit(ADD_operator_c* symbol) {
    return handle_add(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(SUB_operator_c* symbol) {
    return handle_sub(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(MUL_operator_c* symbol) {
    return handle_mul(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(DIV_operator_c* symbol) {
    return handle_div(symbol, prev_il_instruction, il_operand);
}
void* constant_folding_c::visit(MOD_operator_c* symbol) {
    return handle_mod(symbol, prev_il_instruction, il_operand);
}

void* constant_folding_c::visit(GT_operator_c* symbol) {
    handle_cmp(symbol, prev_il_instruction, il_operand, >);
}
void* constant_folding_c::visit(GE_operator_c* symbol) {
    handle_cmp(symbol, prev_il_instruction, il_operand, >=);
}
void* constant_folding_c::visit(EQ_operator_c* symbol) {
    handle_cmp(symbol, prev_il_instruction, il_operand, ==);
}
void* constant_folding_c::visit(LT_operator_c* symbol) {
    handle_cmp(symbol, prev_il_instruction, il_operand, <);
}
void* constant_folding_c::visit(LE_operator_c* symbol) {
    handle_cmp(symbol, prev_il_instruction, il_operand, <=);
}
void* constant_folding_c::visit(NE_operator_c* symbol) {
    handle_cmp(symbol, prev_il_instruction, il_operand, !=);
}

void* constant_folding_c::visit(CAL_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(RET_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(JMP_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(CALC_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(CALCN_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(RETC_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(RETCN_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(JMPC_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
void* constant_folding_c::visit(JMPCN_operator_c* symbol) {
    return handle_move(symbol, prev_il_instruction);
}
...
void* constant_folding_c::visit(or_expression_c* symbol) {
    ...
    return handle_or(symbol, symbol->l_exp, symbol->r_exp);
}
void* constant_folding_c::visit(xor_expression_c* symbol) {
    symbol->l_exp->accept(*this);
    symbol->r_exp->accept(*this);
    return handle_xor(symbol, symbol->l_exp, symbol->r_exp);
}
void* constant_folding_c::visit(and_expression_c* symbol) {
    symbol->l_exp->accept(*this);
    symbol->r_exp->accept(*this);
    return handle_and(symbol, symbol->l_exp, symbol->r_exp);
}

void* constant_folding_c::visit(equ_expression_c* symbol) {
    symbol->l_exp->accept(*this);
    symbol->r_exp->accept(*this);
    handle_cmp(symbol, symbol->l_exp, symbol->r_exp, ==);
}
...
constant_propagation_c::constant_propagation_c(symbol_c* symbol): constant_folding_c(symbol) {
...
static constant_propagation_c::map_values_t inner_left_join_values(constant_propagation_c::map_values_t m1,
                                                                   constant_propagation_c::map_values_t m2) {
    ...
    for (; itr != m1.end(); ++itr) {
        ...
        if (m2.count(name) > 0) {
            ...
            COMPUTE_MEET_SEMILATTICE(int64v, c1, c2, value);
 ...
 *
 *     PROGRAM Prog1_t
 *      VAR_EXTERN COSNTANT  global_const : INT; END_VAR  <--- NOTE: 61131-3 syntax does not allow const value to be set
 * here! VAR .... END_VAR array_var[global_const] := 0; END_PROGRAM
 *
 ...
 *     PROGRAM Prog1_t
 *      VAR_EXTERN COSNTANT  global_const : INT; END_VAR  <--- NOTE: 61131-3 syntax does not allow const value to be set
 * here! VAR_EXTERN           global_array : ARRAY [1..global_const] OF INT; END_VAR VAR .... END_VAR
...
void* constant_propagation_c::handle_var_list_decl(symbol_c* var_list, symbol_c* type_decl, bool is_global_var) {
    type_decl->accept(
        *this);  // Do constant folding of the initial value, and literals in subranges! (we will probably be doing this
                 // multiple times for the same init value, but this is safe as the cvalue is idem-potent)
    ...
    if (itr != function_block_type_symtable.end()) {
        ...
        // WARNING: Before calling fb_type->accept(*this), we must first determine whether we are already currently
        // visiting this exact
        //          same FB declaration (possible with -p option), so we do not get into an infinite loop!!
        // NOTE: We use the std::find() standard algorithm, since the std::stack and std::deque do not have the find()
        // member function.
        //       We could alternatively use a std::set instead of std::deque, but then it would not be evident that
        //       insertion and deletion of fb_types follows a push() and pop() algorithm typical of stacks.
        if (std::find(fbs_currently_being_visited.begin(), fbs_currently_being_visited.end(), fb_type) ==
            fbs_currently_being_visited.end()) {
            // The fb_type is not in the fbs_currently_being_visited stack, so we push it onto the stack, and then visit
            // it!!
    ...
    if (NULL == init_value) {
        return NULL;
    }                           // this is some datatype for which no initial value exists! Do nothing and return.
    init_value->accept(*this);  // necessary when handling default initial values, that were not constant folded in the
                                // call type_decl->accept(*this)
...
bool constant_propagation_c::is_constant(symbol_c* option) {
    return (NULL != dynamic_cast<constant_option_c*>(option));
}
bool constant_propagation_c::is_retain(symbol_c* option) {
    return (NULL != dynamic_cast<retain_option_c*>(option));
}
...
void* constant_propagation_c::visit(var1_init_decl_c* symbol) {
    return handle_var_list_decl(symbol->var1_list, symbol->spec_init);
}
...
void* constant_propagation_c::visit(fb_name_decl_c* symbol) {
    /* A FB has been instantiated inside the POU currently being analysed. We must therefore visit this FB's type
     * declaration and give the VAR_EXTERNs in that FB a chance to get the const values from the global variables
     * currently in scope!
     */
...
// SYM_REF3(input_declarations_c, option, input_declaration_list, method) // Not needed since we inherit from
// iterator_visitor_c!
//  NOTE: Input variables can take any initial value, so we can not set the const_value annotation => we set
//  fixed_init_value to false !!!
//        We must still visit it iteratively, to set the const_value of all literals in the type declarations.
void* constant_propagation_c::visit(input_declarations_c* symbol) {
    return handle_var_decl(symbol->input_declaration_list, false);
}
...
void* constant_propagation_c::visit(output_declarations_c* symbol) {
    return handle_var_decl(symbol->var_init_decl_list, !is_retain(symbol->option) && function_pou_);
}
...
//  fixed_init_value to false !!!
//        We must still visit it iteratively, to set the const_value of all literals in the type declarations.
void* constant_propagation_c::visit(input_output_declarations_c* symbol) {
    return handle_var_decl(symbol->var_declaration_list, false);
}

/* helper symbol for input_output_declarations */
/* var_declaration_list var_declaration ';' */
// SYM_LIST(var_declaration_list_c)                                       // Not needed since we inherit from
// iterator_visitor_c!
...
void* constant_propagation_c::visit(var_declarations_c* symbol) {
    return handle_var_decl(symbol->var_init_decl_list, false);
}

/*  VAR RETAIN var_init_decl_list END_VAR */
// SYM_REF1(retentive_var_declarations_c, var_init_decl_list)             // Not needed since we inherit from
// iterator_visitor_c!
//  NOTE: Retentive variables can take any initial value, so we can not set the const_value annotation => we set
//  fixed_init_value to false !!!
//        We must still visit it iteratively, to set the const_value of all literals in the type declarations.
void* constant_propagation_c::visit(retentive_var_declarations_c* symbol) {
    return handle_var_decl(symbol->var_init_decl_list, false);
}
...
void* constant_propagation_c::visit(external_var_declarations_c* symbol) {
    return handle_var_decl(symbol->external_declaration_list, is_constant(symbol->option));
}
...
void* constant_propagation_c::visit(external_declaration_c* symbol) {
    // The syntax does not allow VAR_EXTERN to be initialized. We must get the initial value from the corresponding
    // VAR_GLOBAL declaration
    /* However, we only do this is if the visit() method for the Program/FB in which this VAR_EXTERN is found was called
     * from the visit(configurtion/resource) visitor! When we are called from the visit(library_c) visitor, we do not
     * have the required information to do this!
     */
    if (NULL != current_configuration) {
        /* before copying the constant value from the VAR_GLOBAL declaration (which is stored in the var_global_values[]
         * map) we check to see if the VAR_EXTERN constant values has already been set by a previous call to constant
         * fold the POU in which this VAR_EXTERN is found we simply check to see if any const value of this VAR_EXTERN
         * variable has already been set previously!
         */
        if (!IS_UNDEFINED(int64v, symbol->specification) || !IS_UNDEFINED(uint64, symbol->specification) ||
             ...
             */
            if (!(symbol->specification->const_value ==
                  var_global_values[get_var_name_c::get_name(symbol->global_var_name)->value]))
                STAGE3_ERROR(0,
                             symbol,
                             symbol,
                             "The initial value of this external variable is ambiguous (the Program/FB in which "
        ...
        symbol->specification
            ->const_value = var_global_values[get_var_name_c::get_name(symbol->global_var_name)->value];
    }
    ...
    // If the datatype specification is a subrange or array, do constant folding of all the literals in that type
    // declaration... (ex: literals in array subrange limits)
    symbol->specification->accept(
        *this);  // should never get to change the const_value of the symbol->specification symbol (only its children!).
    return NULL;
}
 ...
 */
void* constant_propagation_c::visit(global_var_declarations_c* symbol) {
    return handle_var_decl(symbol->global_var_decl_list, is_constant(symbol->option));
}
...
void* constant_propagation_c::visit(function_declaration_c* symbol) {
    ...
    prev_pou_values =
        values;  // store the current values map of whoever called this Function (a program, configuration, or resource)
    values = &local_values;
...
void* constant_propagation_c::visit(function_var_decls_c* symbol) {
    return handle_var_decl(symbol->decl_list, true);
}
...
/* enumvalue_symtable is filled in by enum_declaration_check_c, during stage3 semantic verification, with a list of all
 * enumerated constants declared inside this POU */
// SYM_REF3(function_block_declaration_c, fblock_name, var_declarations, fblock_body, enumvalue_symtable_t
// enumvalue_symtable;)
void* constant_propagation_c::visit(function_block_declaration_c* symbol) {
    map_values_t local_values, *prev_pou_values;
    prev_pou_values =
        values;  // store the current values map of whoever instantited this FB (a program, configuration, or resource)
    values = &local_values;
...
void* constant_propagation_c::visit(temp_var_decls_c* symbol) {
    return handle_var_decl(symbol->var_decl_list, true);
}
...
// NOTE: non_retentive_var_decls_c is only used inside FBs and Programs, so it is safe to call with fixed_init_value_ =
// false
void* constant_propagation_c::visit(non_retentive_var_decls_c* symbol) {
    return handle_var_decl(symbol->var_decl_list, false);
}
...
// SYM_REF3(program_declaration_c, program_type_name, var_declarations, function_block_body, enumvalue_symtable_t
// enumvalue_symtable;)
void* constant_propagation_c::visit(program_declaration_c* symbol) {
    map_values_t local_values, *prev_pou_values;
    prev_pou_values =
        values;  // store the current values map of whoever instantited this Program (a configuration, or resource)
    values = &local_values;
    var_global_values.push(); /* Create inner scope */
...
/* enumvalue_symtable is filled in by enum_declaration_check_c, during stage3 semantic verification, with a list of all
 * enumerated constants declared inside this POU */
// SYM_REF5(configuration_declaration_c, configuration_name, global_var_declarations, resource_declarations,
// access_declarations, instance_specific_initializations,
//          enumvalue_symtable_t enumvalue_symtable; localvar_symbmap_t localvar_symbmap; localvar_symbvec_t
//          localvar_symbvec;)
void* constant_propagation_c::visit(configuration_declaration_c* symbol) {
    ...
    iterator_visitor_c::visit(symbol);  // let the base iterator class handle the rest (basically iterate through the
                                        // whole configuration and do the constant folding!
    current_configuration = NULL;
...
*/
// SYM_REF6(instance_specific_init_c, resource_name, program_name, any_fb_name_list, variable_name, location,
// initialization)
...
void* constant_propagation_c::visit(for_statement_c* symbol) {
    ...
    if (NULL != symbol->by_expression) {
        ...
        if (VALID_CVALUE(int64v, symbol->by_expression) && GET_CVALUE(int64v, symbol->by_expression) > 0 &&
            VALID_CVALUE(int64v, symbol->beg_expression) && VALID_CVALUE(int64v, symbol->end_expression) &&
            GET_CVALUE(int64v, symbol->beg_expression) > GET_CVALUE(int64v, symbol->end_expression))
            return NULL;
        ...
        if (VALID_CVALUE(int64v, symbol->beg_expression) && VALID_CVALUE(int64v, symbol->end_expression) &&
            GET_CVALUE(int64v, symbol->beg_expression) > GET_CVALUE(int64v, symbol->end_expression))
            return NULL;
    }
